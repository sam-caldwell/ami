...
- [X] Object Generation (LLVM → .o)
  ...
  - [X] Compile `.ll` to `.o` for `darwin/arm64` via `clang -target arm64-apple-macosx -c` into `build/obj/<pkg>/<unit>.o`.
  - [X] Update object index to include real `.o` entries (retain `.s` only as debug when verbose).
  ...
  - [X] Diagnostics: `E_TOOLCHAIN_MISSING` (no clang), `E_OBJ_COMPILE_FAIL` with captured stderr in JSON.
...
- [X] Linking (Mach‑O)
  - [X] Link all package objects plus runtime into a single binary under `build/darwin/arm64/<project>` using `clang` with `-target arm64-apple-macosx`.
  - [X] Deterministic flags: strip timestamps/non‑deterministic sections; enable dead‑strip for unused code where safe.
  - [X] Update `ami.manifest` to list produced binaries and their relative paths; keep stable ordering.
