...
func testEmitLLVM_ComparisonsAndIntLiteral(t *testing.T) {
	...
	if !strings.Contains(out, "%t1 = add i64 0, 42") {
		t.Fatalf("literal not lowered: %s", out)
	}
	if !strings.Contains(out, "%t2 = icmp eq i64 %x, %y") {
		t.Fatalf("icmp not lowered: %s", out)
	}
}
...
func testEmitLLVM_Shifts_Neg_Xor_Bnot(t *testing.T) {
	...
	if !strings.Contains(out, "%t1 = shl i64 %x, %y") {
		t.Fatalf("shl not lowered: %s", out)
	}
	if !strings.Contains(out, "%t2 = ashr i64 %x, %y") {
		t.Fatalf("shr not lowered: %s", out)
	}
	if !strings.Contains(out, "%t3 = xor i64 %x, %y") {
		t.Fatalf("xor not lowered: %s", out)
	}
	if !strings.Contains(out, "%t4 = sub i64 0, %x") {
		t.Fatalf("neg not lowered: %s", out)
	}
	if !strings.Contains(out, "%t5 = xor i64 %x, -1") {
		t.Fatalf("bnot not lowered: %s", out)
	}
	if !strings.Contains(out, "%t6 = or i64 %x, %y") {
		t.Fatalf("bor not lowered: %s", out)
	}
}
...
func testEmitLLVM_NotBool(t *testing.T) {
	...
	if !strings.Contains(out, "%t = xor i1 %b, true") {
		t.Fatalf("not not lowered: %s", out)
	}
}
