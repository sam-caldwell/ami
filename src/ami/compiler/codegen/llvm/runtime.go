...
func RuntimeLL(triple string, withMain bool) string {
    ...
    // PHI must be first in block; compute %inext after PHI
    ...
    // Merge next destination and bump j in the same block to avoid extra predecessor
    ...
    s += "llen:\n  %i = phi i64 [ 0, %calc ], [ %inext, %len_accum2 ]\n  %sum = phi i64 [ %sum0, %calc ], [ %sum2, %len_accum2 ]\n  %done = icmp uge i64 %i, %n64\n  br i1 %done, label %alloc, label %lb\n"
    ...
    s += "emit:\n  %j = phi i64 [ 0, %alloc ], [ %jnext, %emitcont ]\n  %dstp = phi ptr [ %dst, %alloc ], [ %nextdst, %emitcont ]\n  %done2 = icmp uge i64 %j, %n64\n  br i1 %done2, label %end, label %emit2\n"
    s += "emit2:\n  %gt0 = icmp ugt i64 %j, 0\n  br i1 %gt0, label %comma, label %noc\n"
    ...
    // Merge next destination and bump j in the same block
    s += "emitcont:\n  %nextdst = phi ptr [ %nextdst_i, %emit_i ], [ %nextdst_d, %emit_d ], [ %nextdst_b, %emit_b ], [ %nextdst_s, %emit_s ], [ %nextdst_o, %emit_o ]\n  %jnext = add i64 %j, 1\n  br label %emit\n"
    ...
    s += "finish:\n  %accv = phi i64 [ %acc, %loop ], [ %acc, %body ]\n  %negv = phi i1 [ %neg, %loop ], [ %neg, %body ]\n  %negval = sub i64 0, %accv\n  %sel = select i1 %negv, i64 %negval, i64 %accv\n  ret i64 %sel\n"
    s += "ret0:\n  ret i64 0\n}\n\n"
    ...
    s += "fracstart:\n  %j2_start = add i64 %j, 1\n  br label %floop\n"
    s += "floop:\n  %k = phi i64 [ %j2_start, %fracstart ], [ %k2, %faccum ]\n  %frac = phi double [ 0.0, %fracstart ], [ %frac2, %faccum ]\n  %scale = phi double [ 1.0, %fracstart ], [ %scale2, %faccum ]\n  %fend = icmp uge i64 %k, %plen\n  br i1 %fend, label %fexit, label %fbody\n"
    s += "fbody:\n  %addr4 = getelementptr i8, ptr %pp, i64 %k\n  %b4 = load i8, ptr %addr4, align 1\n  %ge0f = icmp sge i8 %b4, 48\n  %le9f = icmp sle i8 %b4, 57\n  %isdf = and i1 %ge0f, %le9f\n  br i1 %isdf, label %faccum, label %fexit\n"
    ...
    // Robust object key scanning and JSON value skipper
    // Hex digit to value helper: returns -1 for invalid input
    s += "define i32 @ami_rt_hexval(i8 %c) {\n"
    s += "entry:\n  %ge0 = icmp sge i8 %c, 48\n  %le9 = icmp sle i8 %c, 57\n  %isnum = and i1 %ge0, %le9\n  br i1 %isnum, label %num, label %alpha\n"
    s += "num:\n  %cz = zext i8 %c to i32\n  %d0 = sub i32 %cz, 48\n  ret i32 %d0\n"
    s += "alpha:\n  %gea = icmp sge i8 %c, 97\n  %lef = icmp sle i8 %c, 102\n  %islowa = and i1 %gea, %lef\n  br i1 %islowa, label %la, label %ua\n"
    s += "la:\n  %cz2 = zext i8 %c to i32\n  %sa = sub i32 %cz2, 97\n  %res = add i32 %sa, 10\n  ret i32 %res\n"
    s += "ua:\n  %geaA = icmp sge i8 %c, 65\n  %leFA = icmp sle i8 %c, 70\n  %isupa = and i1 %geaA, %leFA\n  br i1 %isupa, label %ua2, label %bad\n"
    s += "ua2:\n  %cz3 = zext i8 %c to i32\n  %sA = sub i32 %cz3, 65\n  %res2 = add i32 %sA, 10\n  ret i32 %res2\n"
    s += "bad:\n  ret i32 -1\n}\n\n"
    // Match a quoted JSON string at %q (starting at '"') against raw key bytes %key of length %klen.
    // Handles simple escapes (\\x) and decodes Unicode escapes (\\uXXXX and surrogate pairs) to UTF-8,
    // comparing against the UTF-8 bytes in %key.
    s += "define i1 @ami_rt_match_key(ptr %q, ptr %key, i32 %klen) {\n"
    s += "entry:\n  %start = getelementptr i8, ptr %q, i64 1\n  br label %loop\n"
    s += "loop:\n  %pos = phi ptr [ %start, %entry ], [ %nxt, %cont ], [ %npu2, %u2ok ], [ %npu3, %u3ok ], [ %npu4, %u4ok ]\n  %idx = phi i32 [ 0, %entry ], [ %idx2, %cont ], [ %idx2u2, %u2ok ], [ %idx2u3, %u3ok ], [ %idx2u4, %u4ok ]\n  %c = load i8, ptr %pos, align 1\n  %isEsc = icmp eq i8 %c, 92\n  br i1 %isEsc, label %esc, label %chkq\n"
    s += "esc:\n  %pos2 = getelementptr i8, ptr %pos, i64 1\n  %e = load i8, ptr %pos2, align 1\n  %isU = icmp eq i8 %e, 117\n  br i1 %isU, label %uni, label %cmpesc\n"
    s += "cmpesc:\n  %nxt_e = getelementptr i8, ptr %pos, i64 2\n  br label %cmp\n"
    s += "uni:\n  %p3 = getelementptr i8, ptr %pos, i64 2\n  %b1 = load i8, ptr %p3, align 1\n  %p4 = getelementptr i8, ptr %pos, i64 3\n  %b2 = load i8, ptr %p4, align 1\n  %p5 = getelementptr i8, ptr %pos, i64 4\n  %b3 = load i8, ptr %p5, align 1\n  %p6 = getelementptr i8, ptr %pos, i64 5\n  %b4 = load i8, ptr %p6, align 1\n  %v1 = call i32 @ami_rt_hexval(i8 %b1)\n  %v2 = call i32 @ami_rt_hexval(i8 %b2)\n  %v3 = call i32 @ami_rt_hexval(i8 %b3)\n  %v4 = call i32 @ami_rt_hexval(i8 %b4)\n  %m1 = icmp slt i32 %v1, 0\n  %m2 = icmp slt i32 %v2, 0\n  %m3 = icmp slt i32 %v3, 0\n  %m4 = icmp slt i32 %v4, 0\n  %m12 = or i1 %m1, %m2\n  %m34 = or i1 %m3, %m4\n  %mbad = or i1 %m12, %m34\n  br i1 %mbad, label %fail, label %uok\n"
    s += "uok:\n  %t1 = shl i32 %v1, 12\n  %t2 = shl i32 %v2, 8\n  %t3 = shl i32 %v3, 4\n  %t12 = or i32 %t1, %t2\n  %t34 = or i32 %t3, %v4\n  %cp = or i32 %t12, %t34\n  %isAscii = icmp ule i32 %cp, 127\n  br i1 %isAscii, label %uascii, label %umulti\n"
    s += "umulti:\n  %nxt_u_pre = getelementptr i8, ptr %pos, i64 6\n  ; check surrogate ranges\n  %ge_hi = icmp uge i32 %cp, 55296\n  %le_hi = icmp ule i32 %cp, 56319\n  %isHi = and i1 %ge_hi, %le_hi\n  br i1 %isHi, label %sur, label %nonhi\n"
    s += "sur:\n  %bs2 = load i8, ptr %nxt_u_pre, align 1\n  %isbs = icmp eq i8 %bs2, 92\n  br i1 %isbs, label %sur_u, label %fail\n"
    s += "sur_u:\n  %u2p = getelementptr i8, ptr %pos, i64 7\n  %u2ch = load i8, ptr %u2p, align 1\n  %isu = icmp eq i8 %u2ch, 117\n  br i1 %isu, label %sur_hex, label %fail\n"
    s += "sur_hex:\n  %p8 = getelementptr i8, ptr %pos, i64 8\n  %b5 = load i8, ptr %p8, align 1\n  %p9 = getelementptr i8, ptr %pos, i64 9\n  %b6 = load i8, ptr %p9, align 1\n  %p10 = getelementptr i8, ptr %pos, i64 10\n  %b7 = load i8, ptr %p10, align 1\n  %p11 = getelementptr i8, ptr %pos, i64 11\n  %b8 = load i8, ptr %p11, align 1\n  %w1 = call i32 @ami_rt_hexval(i8 %b5)\n  %w2 = call i32 @ami_rt_hexval(i8 %b6)\n  %w3 = call i32 @ami_rt_hexval(i8 %b7)\n  %w4 = call i32 @ami_rt_hexval(i8 %b8)\n  %mw1 = icmp slt i32 %w1, 0\n  %mw2 = icmp slt i32 %w2, 0\n  %mw3 = icmp slt i32 %w3, 0\n  %mw4 = icmp slt i32 %w4, 0\n  %mb12 = or i1 %mw1, %mw2\n  %mb34 = or i1 %mw3, %mw4\n  %mbad2 = or i1 %mb12, %mb34\n  br i1 %mbad2, label %fail, label %sur_ok\n"
    s += "sur_ok:\n  %t1s = shl i32 %w1, 12\n  %t2s = shl i32 %w2, 8\n  %t3s = shl i32 %w3, 4\n  %t12s = or i32 %t1s, %t2s\n  %t34s = or i32 %t3s, %w4\n  %cp_lo = or i32 %t12s, %t34s\n  ; validate low surrogate range DC00..DFFF\n  %ge_lo = icmp uge i32 %cp_lo, 56320\n  %le_lo = icmp ule i32 %cp_lo, 57343\n  %isLo = and i1 %ge_lo, %le_lo\n  br i1 %isLo, label %sur_join, label %fail\n"
    s += "sur_join:\n  %hi_b = sub i32 %cp, 55296\n  %lo_b = sub i32 %cp_lo, 56320\n  %hi10 = shl i32 %hi_b, 10\n  %sum = add i32 %hi10, %lo_b\n  %cpv = add i32 %sum, 65536\n  %npu = getelementptr i8, ptr %pos, i64 12\n  br label %utfsel\n"
    s += "nonhi:\n  ; if low surrogate without high -> invalid\n  %ge_lo2 = icmp uge i32 %cp, 56320\n  %le_lo2 = icmp ule i32 %cp, 57343\n  %isLo2 = and i1 %ge_lo2, %le_lo2\n  br i1 %isLo2, label %fail, label %nonhi2\n"
    s += "nonhi2:\n  %cpv2 = add i32 %cp, 0\n  %npu2_pre = getelementptr i8, ptr %pos, i64 6\n  br label %utfsel\n"
    s += "utfsel:\n  %cp_sel = phi i32 [ %cpv, %sur_join ], [ %cpv2, %nonhi2 ]\n  %npu_sel = phi ptr [ %npu, %sur_join ], [ %npu2_pre, %nonhi2 ]\n  %le7ff = icmp ule i32 %cp_sel, 2047\n  br i1 %le7ff, label %u2, label %chk3\n"
    s += "chk3:\n  %leffff = icmp ule i32 %cp_sel, 65535\n  br i1 %leffff, label %u3, label %chk4\n"
    s += "chk4:\n  %le10ffff = icmp ule i32 %cp_sel, 1114111\n  br i1 %le10ffff, label %u4, label %fail\n"
    s += "u2:\n  %rem2 = sub i32 %klen, %idx\n  %ok2 = icmp uge i32 %rem2, 2\n  br i1 %ok2, label %u2b, label %fail\n"
    s += "u2b:\n  %t02 = lshr i32 %cp_sel, 6\n  %mb02 = or i32 %t02, 192\n  %b02 = trunc i32 %mb02 to i8\n  %msk2 = and i32 %cp_sel, 63\n  %mbx2 = or i32 %msk2, 128\n  %b12 = trunc i32 %mbx2 to i8\n  %k0p2 = getelementptr i8, ptr %key, i32 %idx\n  %k02 = load i8, ptr %k0p2, align 1\n  %eq02 = icmp eq i8 %b02, %k02\n  br i1 %eq02, label %u2c1, label %fail\n"
    s += "u2c1:\n  %idxp1 = add i32 %idx, 1\n  %k1p2 = getelementptr i8, ptr %key, i32 %idxp1\n  %k12 = load i8, ptr %k1p2, align 1\n  %eq12 = icmp eq i8 %b12, %k12\n  br i1 %eq12, label %u2ok, label %fail\n"
    s += "u2ok:\n  %idx2u2 = add i32 %idx, 2\n  %npu2 = getelementptr i8, ptr %npu_sel, i64 0\n  br label %loop\n"
    s += "u3:\n  %rem3 = sub i32 %klen, %idx\n  %ok3 = icmp uge i32 %rem3, 3\n  br i1 %ok3, label %u3b, label %fail\n"
    s += "u3b:\n  %t03 = lshr i32 %cp_sel, 12\n  %mb03 = or i32 %t03, 224\n  %b03 = trunc i32 %mb03 to i8\n  %t13 = lshr i32 %cp_sel, 6\n  %m13 = and i32 %t13, 63\n  %mb13 = or i32 %m13, 128\n  %b13 = trunc i32 %mb13 to i8\n  %t23 = and i32 %cp_sel, 63\n  %mb23 = or i32 %t23, 128\n  %b23 = trunc i32 %mb23 to i8\n  %k0p3 = getelementptr i8, ptr %key, i32 %idx\n  %k03 = load i8, ptr %k0p3, align 1\n  %eq03 = icmp eq i8 %b03, %k03\n  br i1 %eq03, label %u3c1, label %fail\n"
    s += "u3c1:\n  %idxp1_3 = add i32 %idx, 1\n  %k1p3 = getelementptr i8, ptr %key, i32 %idxp1_3\n  %k13 = load i8, ptr %k1p3, align 1\n  %eq13 = icmp eq i8 %b13, %k13\n  br i1 %eq13, label %u3c2, label %fail\n"
    s += "u3c2:\n  %idxp2_3 = add i32 %idx, 2\n  %k2p3 = getelementptr i8, ptr %key, i32 %idxp2_3\n  %k23 = load i8, ptr %k2p3, align 1\n  %eq23 = icmp eq i8 %b23, %k23\n  br i1 %eq23, label %u3ok, label %fail\n"
    s += "u3ok:\n  %idx2u3 = add i32 %idx, 3\n  %npu3 = getelementptr i8, ptr %npu_sel, i64 0\n  br label %loop\n"
    s += "u4:\n  %rem4 = sub i32 %klen, %idx\n  %ok4 = icmp uge i32 %rem4, 4\n  br i1 %ok4, label %u4b, label %fail\n"
    s += "u4b:\n  %t04 = lshr i32 %cp_sel, 18\n  %mb04 = or i32 %t04, 240\n  %b04 = trunc i32 %mb04 to i8\n  %t14 = lshr i32 %cp_sel, 12\n  %m14 = and i32 %t14, 63\n  %mb14 = or i32 %m14, 128\n  %b14 = trunc i32 %mb14 to i8\n  %t24 = lshr i32 %cp_sel, 6\n  %m24 = and i32 %t24, 63\n  %mb24 = or i32 %m24, 128\n  %b24 = trunc i32 %mb24 to i8\n  %msk4 = and i32 %cp_sel, 63\n  %mbx4 = or i32 %msk4, 128\n  %b34 = trunc i32 %mbx4 to i8\n  %k0p4 = getelementptr i8, ptr %key, i32 %idx\n  %k04 = load i8, ptr %k0p4, align 1\n  %eq04 = icmp eq i8 %b04, %k04\n  br i1 %eq04, label %u4c1, label %fail\n"
    s += "u4c1:\n  %idxp1_4 = add i32 %idx, 1\n  %k1p4 = getelementptr i8, ptr %key, i32 %idxp1_4\n  %k14 = load i8, ptr %k1p4, align 1\n  %eq14 = icmp eq i8 %b14, %k14\n  br i1 %eq14, label %u4c2, label %fail\n"
    s += "u4c2:\n  %idxp2_4 = add i32 %idx, 2\n  %k2p4 = getelementptr i8, ptr %key, i32 %idxp2_4\n  %k24 = load i8, ptr %k2p4, align 1\n  %eq24 = icmp eq i8 %b24, %k24\n  br i1 %eq24, label %u4c3, label %fail\n"
    s += "u4c3:\n  %idxp3_4 = add i32 %idx, 3\n  %k3p4 = getelementptr i8, ptr %key, i32 %idxp3_4\n  %k34 = load i8, ptr %k3p4, align 1\n  %eq34 = icmp eq i8 %b34, %k34\n  br i1 %eq34, label %u4ok, label %fail\n"
    s += "u4ok:\n  %idx2u4 = add i32 %idx, 4\n  %npu4 = getelementptr i8, ptr %npu_sel, i64 0\n  br label %loop\n"
    s += "uascii:\n  %cp8 = trunc i32 %cp to i8\n  %nxt_u = getelementptr i8, ptr %pos, i64 6\n  br label %cmp\n"
    s += "chkq:\n  %isQ = icmp eq i8 %c, 34\n  br i1 %isQ, label %end, label %cmplit\n"
    s += "cmplit:\n  %nxt_l = getelementptr i8, ptr %pos, i64 1\n  br label %cmp\n"
    s += "cmp:\n  %val = phi i8 [ %e, %cmpesc ], [ %c, %cmplit ], [ %cp8, %uascii ]\n  %nxt = phi ptr [ %nxt_e, %cmpesc ], [ %nxt_l, %cmplit ], [ %nxt_u, %uascii ]\n  %jend = icmp uge i32 %idx, %klen\n  br i1 %jend, label %fail, label %cmpnext\n"
    s += "cmpnext:\n  %keyptr = getelementptr i8, ptr %key, i32 %idx\n  %kch = load i8, ptr %keyptr, align 1\n  %eq = icmp eq i8 %val, %kch\n  br i1 %eq, label %cont, label %fail\n"
    s += "cont:\n  %idx2 = add i32 %idx, 1\n  br label %loop\n"
    s += "end:\n  %oklen = icmp eq i32 %idx, %klen\n  br i1 %oklen, label %match, label %fail\n"
    s += "match:\n  ret i1 true\n"
    s += "fail:\n  ret i1 false\n}\n\n"

    // Skip a single JSON value starting at %p (after optional whitespace) and return the pointer
    // to the first byte immediately following the value. Handles strings, objects/arrays (with nesting and strings),
    // numbers, and literals true/false/null.
    s += "define ptr @ami_rt_skip_value(ptr %p) {\n"
    s += "entry:\n  %p0 = call ptr @ami_rt_skip_ws(ptr %p)\n  %c0 = load i8, ptr %p0, align 1\n  %isStr = icmp eq i8 %c0, 34\n  br i1 %isStr, label %retstr, label %chkbr\n"
    s += "retstr:\n  %sEnd = call ptr @ami_rt_scan_string_end(ptr %p0)\n  ret ptr %sEnd\n"
    s += "chkbr:\n  %isObj = icmp eq i8 %c0, 123\n  %isArr = icmp eq i8 %c0, 91\n  %isBr = or i1 %isObj, %isArr\n  br i1 %isBr, label %brstart, label %chknum\n"
    s += "brstart:\n  %start = getelementptr i8, ptr %p0, i64 1\n  br label %brloop\n"
    s += "brloop:\n  %h = phi ptr [ %start, %brstart ], [ %hn, %brcont ]\n  %depth = phi i32 [ 1, %brstart ], [ %dcur, %brcont ]\n  %ch = load i8, ptr %h, align 1\n  %isq = icmp eq i8 %ch, 34\n  br i1 %isq, label %brstr, label %brbody\n"
    ...
    s += "brcont:\n  %hn = phi ptr [ %h1, %brstr ], [ %hinc, %brbody ]\n  %dcur = phi i32 [ %depth, %brstr ], [ %d4, %brbody ]\n  %dz = icmp eq i32 %dcur, 0\n  br i1 %dz, label %brdone, label %brloop\n"
    s += "brdone:\n  ret ptr %hn\n"
    s += "chknum:\n  %isMinus = icmp eq i8 %c0, 45\n  %ge0 = icmp sge i8 %c0, 48\n  %le9 = icmp sle i8 %c0, 57\n  %isdig = and i1 %ge0, %le9\n  %isNum = or i1 %isdig, %isMinus\n  br i1 %isNum, label %nloop, label %lit\n"
    s += "nloop:\n  %np = phi ptr [ %p0, %chknum ], [ %np2, %nadv ]\n  %nc = load i8, ptr %np, align 1\n  %nge0 = icmp sge i8 %nc, 48\n  %nle9 = icmp sle i8 %nc, 57\n  %ndig = and i1 %nge0, %nle9\n  %ndot = icmp eq i8 %nc, 46\n  %ne = icmp eq i8 %nc, 101\n  %nE = icmp eq i8 %nc, 69\n  %nplus = icmp eq i8 %nc, 43\n  %nminus = icmp eq i8 %nc, 45\n  %or1 = or i1 %ndig, %ndot\n  %or2 = or i1 %ne, %nE\n  %or3 = or i1 %nplus, %nminus\n  %or4 = or i1 %or1, %or2\n  %keep = or i1 %or4, %or3\n  br i1 %keep, label %nadv, label %ndone\n"
    s += "nadv:\n  %np2 = getelementptr i8, ptr %np, i64 1\n  br label %nloop\n"
    s += "ndone:\n  ret ptr %np\n"
    s += "lit:\n  %ist = icmp eq i8 %c0, 116\n  %isf = icmp eq i8 %c0, 102\n  %isn = icmp eq i8 %c0, 110\n  br i1 %ist, label %lt, label %chkf\n"
    s += "lt:\n  %pt = getelementptr i8, ptr %p0, i64 4\n  ret ptr %pt\n"
    s += "chkf:\n  br i1 %isf, label %lf, label %chkn\n"
    s += "lf:\n  %pf = getelementptr i8, ptr %p0, i64 5\n  ret ptr %pf\n"
    s += "chkn:\n  br i1 %isn, label %ln, label %lother\n"
    s += "ln:\n  %pn = getelementptr i8, ptr %p0, i64 4\n  ret ptr %pn\n"
    s += "lother:\n  ret ptr %p0\n}\n\n"

    // Find the value pointer for key %key within a JSON object at %obj.
    s += "define ptr @ami_rt_object_find(ptr %obj, ptr %key, i32 %klen) {\n"
    s += "entry:\n  %p0 = call ptr @ami_rt_skip_ws(ptr %obj)\n  %c0 = load i8, ptr %p0, align 1\n  %isLB = icmp eq i8 %c0, 123\n  br i1 %isLB, label %start, label %nf\n"
    s += "start:\n  %q0 = getelementptr i8, ptr %p0, i64 1\n  br label %loop\n"
    s += "loop:\n  %pos = phi ptr [ %q0, %start ], [ %nextpos, %skip ], [ %nextpos2, %adv ]\n  %ws = call ptr @ami_rt_skip_ws(ptr %pos)\n  %c = load i8, ptr %ws, align 1\n  %isEnd = icmp eq i8 %c, 125\n  br i1 %isEnd, label %nf, label %checkq\n"
    s += "checkq:\n  %isQ = icmp eq i8 %c, 34\n  br i1 %isQ, label %try, label %adv\n"
    s += "try:\n  %eq = call i1 @ami_rt_match_key(ptr %ws, ptr %key, i32 %klen)\n  %aft = call ptr @ami_rt_scan_string_end(ptr %ws)\n  %val = call ptr @ami_rt_after_colon(ptr %aft)\n  br i1 %eq, label %found, label %skip\n"
    s += "found:\n  ret ptr %val\n"
    s += "skip:\n  %aft2 = call ptr @ami_rt_skip_value(ptr %val)\n  %ws2 = call ptr @ami_rt_skip_ws(ptr %aft2)\n  %c2 = load i8, ptr %ws2, align 1\n  %isComma = icmp eq i8 %c2, 44\n  %commaNext = getelementptr i8, ptr %ws2, i64 1\n  %nextpos = select i1 %isComma, ptr %commaNext, ptr %ws2\n  br label %loop\n"
    s += "adv:\n  %nextpos2 = getelementptr i8, ptr %pos, i64 1\n  br label %loop\n"
    s += "nf:\n  ret ptr null\n}\n\n"
    ...
    s += "ret_arr:\n  ret ptr %val_arr\n"
    s += "objcase:\n  %val_obj = call ptr @ami_rt_object_find(ptr %b, ptr %p, i32 %seglen)\n  %isnull2 = icmp eq ptr %val_obj, null\n  br i1 %isnull2, label %notfound, label %afterobj\n"
    s += "afterobj:\n  %atend2 = icmp eq i32 %seglen, %r\n  br i1 %atend2, label %ret_obj, label %objcont\n"
    s += "ret_obj:\n  ret ptr %val_obj\n"
    s += "objcont:\n  br label %contnest\n"
    s += "contnest:\n  %bnext = phi ptr [ %val_arr, %iget ], [ %val_obj, %objcont ]\n  %segp = add i32 %seglen, 1\n  %pnext = getelementptr i8, ptr %p, i32 %segp\n  %rnext = sub i32 %r, %segp\n  br label %segloop\n"
    s += "notfound:\n  ret ptr null\n}\n\n"
