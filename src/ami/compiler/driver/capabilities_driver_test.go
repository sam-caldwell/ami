...
func TestCompile_Capabilities_IoRequiresCapability(t *testing.T) {
    ...
    found := false
    for _, d := range ds { if d.Code == "E_CAPABILITY_REQUIRED" { found = true } }
    if !found { t.Fatalf("expected E_CAPABILITY_REQUIRED; got %+v", ds) }
}
...
func TestCompile_Capabilities_TrustUntrustedForbidsIo(t *testing.T) {
    ...
    _, ds := Compile(ws, pkgs, Options{Debug: false})
    found := false
    for _, d := range ds { if d.Code == "E_TRUST_VIOLATION" { found = true } }
    if !found { t.Fatalf("expected E_TRUST_VIOLATION; got %+v", ds) }
}
