...
func TestCompile_CrossUnit_SignatureMismatches(t *testing.T) {
    ...
    for _, d := range diags { if d.Code == "E_CALL_ARG_TYPE_MISMATCH" { hasArg = true }; if d.Code == "E_CALL_ARITY_MISMATCH" { hasArity = true } }
    if !hasArg || !hasArity { t.Fatalf("missing cross-unit call diagnostics: %v", diags) }
}
