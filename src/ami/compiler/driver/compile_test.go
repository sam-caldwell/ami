...
func testCompile_MinimalFunction_IRDebugAndNoDiags(t *testing.T) {
	...
	if len(arts.IR) != 1 {
		t.Fatalf("expected 1 IR file, got %d", len(arts.IR))
	}
	out := arts.IR[0]
	...
	if obj["package"] != "app" {
		t.Fatalf("package: %v", obj["package"])
	}
	...
	if err := json.Unmarshal(pb, &pobj); err != nil {
		t.Fatalf("pipelines json: %v", err)
	}
	if pobj["schema"] != "pipelines.v1" {
		t.Fatalf("schema: %v", pobj["schema"])
	}
	...
	if err != nil {
		t.Fatalf("read eventmeta: %v", err)
	}
	var emobj map[string]any
	if err := json.Unmarshal(eb, &emobj); err != nil {
		t.Fatalf("eventmeta json: %v", err)
	}
	if emobj["schema"] != "eventmeta.v1" {
		t.Fatalf("schema: %v", emobj["schema"])
	}
...
func testCompile_MemSafetyDiagnostics(t *testing.T) {
	...
	if len(diags) == 0 {
		t.Fatalf("expected diagnostics for mem safety")
	}
	...
	if !hasPos {
		t.Fatalf("expected at least one diagnostic with file and position; got: %+v", diags)
	}
}
...
func testCompile_PipelineSemanticsDiagnostics(t *testing.T) {
	...
	if !hasStart || !hasUnknown {
		t.Fatalf("missing expected codes: %v", diags)
	}
	// Positions present
	for _, d := range diags {
...
func testCompile_PipelinesDebug_AttrsRaw(t *testing.T) {
	...
	var pobj map[string]any
	if err := json.Unmarshal(b, &pobj); err != nil {
		t.Fatalf("json: %v", err)
	}
	pipes, _ := pobj["pipelines"].([]any)
	if len(pipes) != 1 {
		t.Fatalf("pipes len: %d", len(pipes))
	}
	...
	if a0["name"] != "edge.MultiPath" {
		t.Fatalf("attr name: %v", a0["name"])
	}
	av, _ := a0["args"].([]any)
	if len(av) != 1 || av[0].(string) != "merge.Stable()" {
...
func testCompile_IRTyping_ReduceAnyOnReturn(t *testing.T) {
	...
	if last["op"] != "RETURN" {
		t.Fatalf("last op not RETURN: %v", last["op"])
	}
	vals := last["values"].([]any)
	if len(vals) != 1 {
		t.Fatalf("want 1 return value")
	}
	v0 := vals[0].(map[string]any)
	if v0["type"] != "int" {
		t.Fatalf("want return type int, got %v", v0["type"])
	}
}
...
func testCompile_IRTyping_VarInit_InferredType(t *testing.T) {
	...
	if !foundVar {
		t.Fatalf("expected VAR with type=int")
	}
...
func testCompile_IRTyping_CallResultType(t *testing.T) {
	...
	v0 := vals[0].(map[string]any)
	if v0["type"] != "int" {
		t.Fatalf("return type: %v", v0["type"])
	}
}
...
func testCompile_EdgesIndexDebug(t *testing.T) {
	...
	if obj["schema"] != "edges.v1" {
		t.Fatalf("schema: %v", obj["schema"])
	}
	edgesArr := obj["edges"].([]any)
	if len(edgesArr) == 0 {
		t.Fatalf("no edges in index")
	}
	e0 := edgesArr[0].(map[string]any)
	if _, ok := e0["bounded"]; !ok {
		t.Fatalf("bounded missing")
	}
	if _, ok := e0["delivery"]; !ok {
		t.Fatalf("delivery missing")
	}
	if e0["bounded"] != true || e0["delivery"] != "bestEffort" {
		t.Fatalf("edge derivation wrong: %+v", e0)
	}
	if e0["type"] != "X" {
		t.Fatalf("edge type not propagated: %+v", e0)
	}
	// multipath collect snapshot present
	if obj["collect"] == nil {
		t.Fatalf("collect snapshot missing")
	}
	col := obj["collect"].([]any)
	if len(col) == 0 {
		t.Fatalf("collect entries empty")
	}
}
...
func testCompile_SourcesDebug_ImportsDetailed(t *testing.T) {
	...
	if !ok || len(imp) != 2 {
		t.Fatalf("importsDetailed: %T len=%d", obj["importsDetailed"], len(imp))
	}
}
...
func testCompile_ASTDebug_FileExistsAndSchema(t *testing.T) {
	...
	if !ok || len(imps) != 1 {
		t.Fatalf("imports: %T len=%d", obj["imports"], len(imps))
	}
	imp0, _ := imps[0].(map[string]any)
	if imp0["path"] != "alpha" || imp0["constraint"] != ">= v1.0.0" {
		t.Fatalf("import0: %+v", imp0)
	}
	// funcs content
	fns, ok := obj["funcs"].([]any)
	if !ok || len(fns) != 1 {
		t.Fatalf("funcs: %T len=%d", obj["funcs"], len(fns))
	}
	fn0, _ := fns[0].(map[string]any)
	if fn0["name"] != "F" {
		t.Fatalf("func name: %v", fn0["name"])
	}
	tps, _ := fn0["typeParams"].([]any)
	if len(tps) != 1 {
		t.Fatalf("typeParams len: %d", len(tps))
	}
	tp0, _ := tps[0].(map[string]any)
	if tp0["name"] != "T" || tp0["constraint"] != "any" {
		t.Fatalf("typeParam0: %+v", tp0)
	}
	// pipelines content
	pipes, ok := obj["pipelines"].([]any)
	if !ok || len(pipes) != 1 {
		t.Fatalf("pipelines: %T len=%d", obj["pipelines"], len(pipes))
	}
	p0, _ := pipes[0].(map[string]any)
	if p0["name"] != "P" {
		t.Fatalf("pipe name: %v", p0["name"])
	}
	steps, _ := p0["steps"].([]any)
	if len(steps) != 1 {
