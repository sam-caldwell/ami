...
func testDiagnostics_JSON_UnresolvedIdent_IncludesFields(t *testing.T) {
	...
	var d diag.Record
	for _, r := range diags {
		if r.Code == "E_UNRESOLVED_IDENT" {
			d = r
			break
		}
	}
	if d.Code == "" {
		t.Fatalf("E_UNRESOLVED_IDENT not found: %+v", diags)
	}
	b := diag.Line(d)
	s := string(b)
	if !strings.Contains(s, "\"schema\":\"diag.v1\"") {
	...
	if !strings.Contains(s, "\"code\":\"E_UNRESOLVED_IDENT\"") {
		t.Fatalf("missing code: %s", s)
	}
	if !strings.Contains(s, "\"message\":\"unresolved identifier") {
		t.Fatalf("missing message: %s", s)
	}
	if !strings.Contains(s, "\"file\":\"u.ami\"") {
		t.Fatalf("missing file: %s", s)
	}
	if !strings.Contains(s, "\"pos\":{") {
		t.Fatalf("missing pos: %s", s)
	}
}
