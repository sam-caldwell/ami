...
func TestDriver_PragmaTelemetry_FlowsIntoASM(t *testing.T) {
    dir := t.TempDir()
    src := "#pragma telemetry trace,counters\npackage p\nfunc main(){}\n"
    ...
    asm := res.ASM[0].Text
    if !strings.Contains(asm, "; telemetry trace,counters") {
        t.Fatalf("asm missing telemetry header: %q", asm)
    }
}
