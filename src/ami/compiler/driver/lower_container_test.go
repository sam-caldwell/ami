...
func TestCompile_ContainerLiteral_AssignAndVarInit_TypesInIR(t *testing.T) {
    ...
    if len(diags) != 0 { t.Fatalf("diags: %+v", diags) }
    if len(arts.IR) != 1 { t.Fatalf("expected 1 IR artifact") }
    ...
    if len(instrs) < 4 { t.Fatalf("expected several instrs, got %d", len(instrs)) }
    ...
    for _, it := range instrs {
        ...
        case "ASSIGN":
            ...
            if src["type"] == "slice<int>" { foundSlice = true }
            if src["type"] == "map<string,int>" { foundMap = true }
        case "VAR":
            if init, ok := m["init"].(map[string]any); ok {
                if init["type"] == "slice<int>" { foundSlice = true }
                if init["type"] == "map<string,int>" { foundMap = true }
            }
        }
    }
    if !foundSlice { t.Fatalf("slice literal type not found in IR") }
    if !foundMap { t.Fatalf("map literal type not found in IR") }
}
...
func TestCompile_ContainerLiteral_Set_TypeInIR(t *testing.T) {
    ...
    for _, it := range instrs {
        ...
        if m["op"] == "ASSIGN" {
            src := m["src"].(map[string]any)
            if src["type"] == "set<string>" { found = true }
        }
    }
    if !found { t.Fatalf("set literal type not found in IR") }
}
