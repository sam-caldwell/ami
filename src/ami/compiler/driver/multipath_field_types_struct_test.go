...
func TestCompile_MergeField_Struct_ResolveAndOrderable(t *testing.T) {
    ...
    fs.AddFile("u.ami", "package app\npipeline P(){ A type(\"Event<Struct{id:int,ts:int}>\"); A -> Collect; Collect merge.Sort(\"ts\"); egress }\n")
...
func TestCompile_MergeField_Struct_Unknown(t *testing.T) {
    ws := workspace.Workspace{}
    fs := &source.FileSet{}
    fs.AddFile("u.ami", "package app\npipeline P(){ A type(\"Event<Struct{id:int}>\"); A -> Collect; Collect merge.Sort(\"missing\"); egress }\n")
    pkgs := []Package{{Name: "app", Files: fs}}
    _, ds := Compile(ws, pkgs, Options{Debug: false})
    has := false
...
func TestCompile_MergeField_Struct_Unorderable(t *testing.T) {
    ...
    fs.AddFile("u.ami", "package app\npipeline P(){ A type(\"Event<Struct{obj:Struct{x:int}}>>\"); A -> Collect; Collect merge.Sort(\"obj\"); egress }\n")
    pkgs := []Package{{Name: "app", Files: fs}}
    _, ds := Compile(ws, pkgs, Options{Debug: false})
    has := false
