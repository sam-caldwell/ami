...
func TestCompile_MergeField_OnPrimitive_Error(t *testing.T) {
    ...
    has := false
    for _, d := range ds { if d.Code == "E_MERGE_FIELD_ON_PRIMITIVE" { has = true } }
    if !has { t.Fatalf("expected E_MERGE_FIELD_ON_PRIMITIVE; got %+v", ds) }
}
...
func TestCompile_MergeField_Unverified_Warn(t *testing.T) {
    ...
    _, ds := Compile(ws, pkgs, Options{Debug: false})
    has := false
    for _, d := range ds { if d.Code == "W_MERGE_FIELD_UNVERIFIED" { has = true } }
    if !has { t.Fatalf("expected W_MERGE_FIELD_UNVERIFIED; got %+v", ds) }
}
