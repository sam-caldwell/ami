...
func TestLower_VarInit_ShortCircuit_Bool(t *testing.T) {
    ...
    s := string(bts)
    if !strings.Contains(s, "br i1 ") { t.Fatalf("missing cond br for var init:\n%s", s) }
    if !strings.Contains(s, "phi i1") { t.Fatalf("missing phi i1 for var init:\n%s", s) }
}
...
func TestLower_VarInit_ShortCircuit_Owned_String(t *testing.T) {
    ...
    if !strings.Contains(s, "br i1 ") { t.Fatalf("missing cond br in Owned var init:\n%s", s) }
    if !strings.Contains(s, "declare i64 @ami_rt_string_len(ptr)") { t.Fatalf("missing extern for string_len:\n%s", s) }
    if !strings.Contains(s, "call i64 @ami_rt_string_len(") { t.Fatalf("missing call to string_len:\n%s", s) }
    if !strings.Contains(s, "call ptr @ami_rt_owned_new(") { t.Fatalf("missing call to owned_new:\n%s", s) }
}
