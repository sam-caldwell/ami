...
func TestFIFO_PushPop_FIFOOrder(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		if err := fq.Push(Event{Payload: i}); err != nil {
...
fq := &FIFO{MaxCapacity: 2, Backpressure: BackpressureDropOldest}
	...
	if !ok || ev.Payload.(int) != 2 {
		t.Fatalf("expect first=2 got=%v ok=%v", ev.Payload, ok)
	}
	ev, ok = fq.Pop()
	if !ok || ev.Payload.(int) != 3 {
		t.Fatalf("expect second=3 got=%v ok=%v", ev.Payload, ok)
	}
}

func TestFIFO_Backpressure_BlockReturnsErr(t *testing.T) {
	fq := &FIFO{MaxCapacity: 1, Backpressure: BackpressureBlock}
	if err := fq.Push(Event{Payload: 1}); err != nil {
		t.Fatalf("push err: %v", err)
	}
	if err := fq.Push(Event{Payload: 2}); err == nil {
