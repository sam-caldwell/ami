...
func TestPipeline_PushPop_FIFOOrder(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		_ = q.Push(Event{Payload: i})
	}
	for i := 1; i <= 3; i++ {
		ev, ok := q.Pop()
		if !ok || ev.Payload.(int) != i {
			t.Fatalf("want %d got %v ok=%v", i, ev.Payload, ok)
		}
	}
}
...
func TestPipeline_Backpressure_DropOldest(t *testing.T) {
	...
	if !ok || ev.Payload.(int) != 2 {
		t.Fatalf("first want 2 got %v ok=%v", ev.Payload, ok)
	}
	ev, ok = q.Pop()
	if !ok || ev.Payload.(int) != 3 {
		t.Fatalf("second want 3 got %v ok=%v", ev.Payload, ok)
	}
}
...
func TestPipeline_BlockReturnsErrOnFull(t *testing.T) {
	...
	_ = q.Push(Event{Payload: 1})
	if err := q.Push(Event{Payload: 2}); err == nil {
