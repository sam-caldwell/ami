...
func TestFIFOQueue_Counters(t *testing.T) {
    ...
    _ = q.Push(1); _ = q.Push(2); _ = q.Push(3) // dropNewest
    ...
    _ = qb.Push(9)
    _ = qb.Push(10)
    if _, _, _, full := qb.Counters(); full != 1 { t.Fatalf("fifo full count: %d", full) }
}

func TestLIFOStack_Counters(t *testing.T) {
    s, _ := NewLIFO(LIFO{MaxCapacity: 2, Backpressure: "dropOldest"})
    _ = s.Push(1); _ = s.Push(2); _ = s.Push(3) // drop oldest
    ...
    _ = sb.Push(9)
    _ = sb.Push(10)
    if _, _, _, full := sb.Counters(); full != 1 { t.Fatalf("lifo full count: %d", full) }
}
