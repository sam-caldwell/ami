...
func testFIFOQueue_OrderAndBackpressure(t *testing.T) {
	...
	a, ok := q.Pop()
	if !ok || a.(int) != 1 {
		t.Fatalf("pop a: %v %v", a, ok)
	}
	b, ok := q.Pop()
	if !ok || b.(int) != 2 {
		t.Fatalf("pop b: %v %v", b, ok)
	}
	if _, ok := q.Pop(); ok {
...
func testEdgeRuntime_SimpleConcurrency(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		v, ok := q.Pop()
		if !ok || v.(int) != i {
			t.Fatalf("fifo %d got %v ok=%v", i, v, ok)
		}
	}
	// LIFO
	...
	for i := N - 1; i >= 0; i-- {
		v, ok := s.Pop()
		if !ok || v.(int) != i {
			t.Fatalf("lifo %d got %v ok=%v", i, v, ok)
		}
	}
}
