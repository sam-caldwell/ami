...
func TestEncode_ExprCall_MultiResult_EncodesResultsAndRetTypes(t *testing.T) {
    ...
    if !ok || len(rsAny) != 2 { t.Fatalf("results missing or wrong len: %#v", expr["results"]) }
    ...
    rt := expr["retTypes"].([]any)
    if len(rt) != 2 || rt[0] != "int" || rt[1] != "string" { t.Fatalf("retTypes: %+v", rt) }
    // Verify sig.results mirrors result types
    sig := expr["sig"].(map[string]any)
    sigRs := sig["results"].([]any)
    if len(sigRs) != 2 || sigRs[0] != "int" || sigRs[1] != "string" { t.Fatalf("sig.results: %+v", sigRs) }
}
