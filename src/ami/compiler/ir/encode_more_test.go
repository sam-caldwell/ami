...
func testEncodeModule_CoversTopLevelAndInstrs(t *testing.T) {
	...
	if obj["concurrency"].(float64) != 4 {
		t.Fatalf("concurrency: %v", obj["concurrency"])
	}
	if obj["backpressurePolicy"].(string) != "dropOldest" {
		t.Fatalf("backpressurePolicy: %v", obj["backpressurePolicy"])
	}
	if obj["telemetryEnabled"].(bool) != true {
		t.Fatalf("telemetryEnabled: %v", obj["telemetryEnabled"])
	}
	if tr, ok := obj["trustLevel"].(string); !ok || tr == "" {
		t.Fatalf("trustLevel: %v", obj["trustLevel"])
	}
	// Inspect function call signature block
	funs := obj["functions"].([]any)
	if len(funs) != 1 {
		t.Fatalf("functions len: %d", len(funs))
	}
	...
	for _, in := range instrs {
		...
		if im["op"] == "EXPR" {
			...
			if e["op"] == "call" {
				if _, ok := e["sig"].(map[string]any); !ok {
					t.Fatalf("missing sig in call: %v", e)
				}
				hasCall = true
			}
		}
