...
func TestIR_ToSchema_ParamAnnotations_OwnershipAndDomain(t *testing.T) {
	...
	for _, p := range params {
		byName[p.Name] = schParam{Domain: p.Domain, Ownership: p.Ownership}
	}
	// Event param → domain=event, ownership=borrowed
	if v := byName["ev"]; v.Domain != "event" || v.Ownership != "borrowed" {
		t.Fatalf("event annotations: %+v", v)
	}
	// Owned<T> param → ownership=owned, domain=ephemeral by default
	if v := byName["r"]; v.Ownership != "owned" || v.Domain != "ephemeral" {
		t.Fatalf("owned annotations: %+v", v)
	}
	// State param (ptr or not) → domain=state
	if v := byName["st"]; v.Domain != "state" {
		t.Fatalf("state(ptr) annotation: %+v", v)
	}
	if v := byName["st2"]; v.Domain != "state" {
		t.Fatalf("state(val) annotation: %+v", v)
	}
	// Ephemeral default for regular params
	if v := byName["x"]; v.Domain != "ephemeral" {
		t.Fatalf("ephemeral annotation: %+v", v)
	}
}
