...
func TestLowerPipelines_CapturesGenericPayloads(t *testing.T) {
    ...
    m.LowerPipelines(f)
    if len(m.Pipelines) != 1 { t.Fatalf("expected 1 pipeline; got %d", len(m.Pipelines)) }
    if len(m.Pipelines[0].Steps) != 1 { t.Fatalf("expected 1 step; got %d", len(m.Pipelines[0].Steps)) }
    w := m.Pipelines[0].Steps[0].Workers
    if len(w) != 1 { t.Fatalf("expected 1 worker; got %d", len(w)) }
