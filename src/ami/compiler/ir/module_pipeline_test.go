...
func TestLowerPipelines_WorkerContextState(t *testing.T) {
    ...
    m.LowerPipelines(f)
    if len(m.Pipelines) != 1 || len(m.Pipelines[0].Steps) != 1 { t.Fatalf("unexpected pipelines shape: %+v", m.Pipelines) }
    w := m.Pipelines[0].Steps[0].Workers
    if len(w) != 1 || !w[0].HasContext || !w[0].HasState { t.Fatalf("worker flags incorrect: %+v", w) }
}
