...
func TestValidatePackageIdent(t *testing.T) {
    ...
    for _, s := range good {
        if !ValidatePackageIdent(s) { t.Fatalf("expected valid: %q", s) }
    }
    for _, s := range bad {
        if ValidatePackageIdent(s) { t.Fatalf("expected invalid: %q", s) }
    }
}
...
func TestValidateImportPath(t *testing.T) {
    ...
    for _, s := range good {
        if !ValidateImportPath(s) { t.Fatalf("expected valid import: %q", s) }
    }
    for _, s := range bad {
        if ValidateImportPath(s) { t.Fatalf("expected invalid import: %q", s) }
    }
}
