...
// skipSpaceAndComments consumes whitespace and both line and block comments,
// updating line/column and capturing comment text into s.pending.
        ...
        if r == '/' && s.peekRune() == '*' {
            ...
            if idx := strings.Index(s.src[s.off:], "*/"); idx >= 0 {
                ...
                if nl := strings.Count(segment, string(tok.LexLf)); nl > 0 {
                    s.line += nl
                    // compute tail length after last newline to set accurate column
                    last := strings.LastIndex(segment, string(tok.LexLf))
                    tailLen := len(segment) - last - 1
                    s.column = 1 + tailLen
                } else {
