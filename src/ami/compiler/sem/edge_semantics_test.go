...
`
    ...
    res := AnalyzeFile(f)
    found := false
    for _, d := range res.Diagnostics { if d.Code == "E_EDGE_BP_INVALID" { found = true; break } }
    if !found { t.Fatalf("expected E_EDGE_BP_INVALID; diags=%v", res.Diagnostics) }
}
...
`
    ...
    res := AnalyzeFile(f)
    found := false
    for _, d := range res.Diagnostics { if d.Code == "E_EDGE_CAP_ORDER" { found = true; break } }
    if !found { t.Fatalf("expected E_EDGE_CAP_ORDER; diags=%v", res.Diagnostics) }
}
...
`
    ...
    res := AnalyzeFile(f)
    found := false
    for _, d := range res.Diagnostics { if d.Code == "E_EDGE_MINCAP" { found = true; break } }
    if !found { t.Fatalf("expected E_EDGE_MINCAP; diags=%v", res.Diagnostics) }
}
...
`
    ...
    res := AnalyzeFile(f)
    found := false
    for _, d := range res.Diagnostics { if d.Code == "E_EDGE_NAME_REQUIRED" { found = true; break } }
    if !found { t.Fatalf("expected E_EDGE_NAME_REQUIRED; diags=%v", res.Diagnostics) }
}
