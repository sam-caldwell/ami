...
pipeline P { Ingress(cfg).Transform(f).Egress(in=edge.FIFO(minCapacity=0,maxCapacity=0,backpressure=block,type=int)) }`
	...
	for _, d := range res.Diagnostics {
		if d.Code == "E_EDGE_TYPE_MISMATCH" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_EDGE_TYPE_MISMATCH; diags=%v", res.Diagnostics)
	}
}
...
pipeline P { Ingress(cfg).Transform(f).Egress(in=edge.FIFO(minCapacity=0,maxCapacity=0,backpressure=block,type=string)) }`
	...
	for _, d := range res.Diagnostics {
		if d.Code == "E_EDGE_TYPE_MISMATCH" {
			t.Fatalf("unexpected type mismatch: %v", d)
		}
	}
}
...
pipeline B { Ingress(cfg).Egress(in=edge.Pipeline(name=A)) }`
	...
	var found bool
	for _, d := range res.Diagnostics {
		if d.Code == "E_CYCLE_DETECTED" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_CYCLE_DETECTED; diags=%v", res.Diagnostics)
	}
}
...
pipeline B { Ingress(cfg).Egress(in=edge.Pipeline(name=A)) }`
	...
	for _, d := range res.Diagnostics {
		if d.Code == "E_CYCLE_DETECTED" {
			t.Fatalf("unexpected cycle diag: %v", d)
		}
	}
}
