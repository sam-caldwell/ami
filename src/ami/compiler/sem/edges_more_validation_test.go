...
func TestEdges_UnknownParam_Error(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "E_EDGE_PARAM_UNKNOWN" { has = true } }
    if !has { t.Fatalf("expected E_EDGE_PARAM_UNKNOWN, got %v", ds) }
}
...
func TestEdges_MaxNegative_Error(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "E_EDGE_CAPACITY_INVALID" { has = true } }
    if !has { t.Fatalf("expected E_EDGE_CAPACITY_INVALID for max<0, got %v", ds) }
}
...
func TestEdges_Backpressure_InvalidPolicy(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "E_EDGE_BACKPRESSURE" { has = true } }
    if !has { t.Fatalf("expected E_EDGE_BACKPRESSURE for invalid value, got %v", ds) }
}
