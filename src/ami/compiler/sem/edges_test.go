...
func TestEdges_FIFO_MinMaxOrder_Err(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "E_EDGE_CAPACITY_ORDER" { has = true } }
    if !has { t.Fatalf("expected E_EDGE_CAPACITY_ORDER, got %v", ds) }
}
...
func TestEdges_Pipeline_NotFound(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "E_EDGE_PIPE_NOT_FOUND" { has = true } }
    if !has { t.Fatalf("expected E_EDGE_PIPE_NOT_FOUND, got %v", ds) }
}
...
func TestEdges_Pipeline_TypeMismatch(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "E_EDGE_PIPE_TYPE_MISMATCH" { has = true } }
    if !has { t.Fatalf("expected E_EDGE_PIPE_TYPE_MISMATCH, got %v", ds) }
}
...
func TestEdges_Backpressure_LegacyDrop_Warns(t *testing.T) {
    ...
    ds := AnalyzeEdges(af)
    has := false
    for _, d := range ds { if d.Code == "W_EDGE_BP_LEGACY_DROP" { has = true } }
    if !has { t.Fatalf("expected legacy drop warning, got %v", ds) }
}
