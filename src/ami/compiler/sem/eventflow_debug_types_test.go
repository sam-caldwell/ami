                ...
                if st.Name == "A" || st.Name == "B" {
                    for _, at := range st.Attrs {
                        if (at.Name == "type" || at.Name == "Type") && len(at.Args) > 0 {
                            if st.Name == "A" { ta = at.Args[0].Text } else { tb = at.Args[0].Text }
                        }
                    }
                }
            case *ast.EdgeStmt:
                if st.From == "A" && st.To == "B" { haveEdge = true }
            }
        }
    }
    if ta != "Event<string>" { t.Fatalf("A type missing or wrong: %q", ta) }
    if tb != "Event<Union<int,int64>>" { t.Fatalf("B type missing or wrong: %q", tb) }
    if !haveEdge { t.Fatalf("missing edge A->B") }
}
...
func TestDebug_AnalyzeEventTypeFlow_UnionMismatch(t *testing.T) {
    ...
    found := false
    for _, d := range ds { if d.Code == "E_EVENT_TYPE_FLOW" { found = true } }
    if !found { t.Fatalf("expected union mismatch diagnostic; got %v", ds) }
}
