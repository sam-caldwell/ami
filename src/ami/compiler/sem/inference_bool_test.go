...
func TestTypeInference_Bool_From_Equality(t *testing.T) {
    ...
    for _, d := range ds {
        if d.Code == "E_TYPE_MISMATCH" { t.Fatalf("unexpected mismatch: %+v", ds) }
    }
}
...
func TestTypeInference_Bool_Mismatch_AssignedToInt(t *testing.T) {
    ...
    found := false
    for _, d := range ds {
        if d.Code == "E_TYPE_MISMATCH" { found = true; break }
    }
    if !found { t.Fatalf("expected type mismatch for assigning bool to int") }
}
