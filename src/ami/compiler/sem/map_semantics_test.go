...
`
	...
	found := false
	for _, d := range res.Diagnostics {
		if d.Code == "E_MAP_ARITY" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_MAP_ARITY; diags=%v", res.Diagnostics)
	}
}
...
`
	...
	found := false
	for _, d := range res.Diagnostics {
		if d.Code == "E_MAP_KEY_TYPE_INVALID" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_MAP_KEY_TYPE_INVALID for slice key; diags=%v", res.Diagnostics)
	}
}
...
`
	...
	if !found {
		t.Fatalf("expected E_MAP_KEY_TYPE_INVALID for map key; diags=%v", res.Diagnostics)
	}
}
...
`
	...
	if !found {
		t.Fatalf("expected E_MAP_KEY_TYPE_INVALID for pointer key; diags=%v", res.Diagnostics)
	}
}
