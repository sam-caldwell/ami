...
func testMemSafety_BanAmpersand(t *testing.T) {
	...
	if len(ds) != 1 {
		t.Fatalf("want 1 diag, got %d: %s", len(ds), diagJSON(ds))
	}
	if ds[0].Code != "E_PTR_UNSUPPORTED_SYNTAX" {
...
func testMemSafety_UnaryStarOnlyOnLHS(t *testing.T) {
	...
	ds := AnalyzeMemorySafety(f)
	if len(ds) != 0 {
		t.Fatalf("want 0 diags, got %d: %s", len(ds), diagJSON(ds))
	}
}
...
func testMemSafety_UnaryStarElsewhere_IsError(t *testing.T) {
	...
	if ds[0].Code != "E_MUT_BLOCK_UNSUPPORTED" {
		t.Fatalf("code: %s", ds[0].Code)
	}
}
