...
func TestAnalyzeMultiPath_SortPrimaryMustEqualKey_Err(t *testing.T) {
    ...
    has := false
    for _, d := range ds { if d.Code == "E_MERGE_SORT_PRIMARY_NEQ_KEY" { has = true } }
    if !has { t.Fatalf("expected E_MERGE_SORT_PRIMARY_NEQ_KEY, got %+v", ds) }
}
...
func TestAnalyzeMultiPath_SortPrimaryMustEqualPartition_Err(t *testing.T) {
    ...
    ds := AnalyzeMultiPath(af)
    has := false
    for _, d := range ds { if d.Code == "E_MERGE_SORT_PRIMARY_NEQ_PARTITION" { has = true } }
    if !has { t.Fatalf("expected E_MERGE_SORT_PRIMARY_NEQ_PARTITION, got %+v", ds) }
}
