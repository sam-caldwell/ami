...
func TestMerge_DedupFieldUnderPartition_WarnsByDefault(t *testing.T) {
    ...
    found := false
    for _, d := range ds { if d.Code == "W_MERGE_DEDUP_FIELD_WITHOUT_KEY_UNDER_PARTITION" { found = true; break } }
    if !found { t.Fatalf("expected W_MERGE_DEDUP_FIELD_WITHOUT_KEY_UNDER_PARTITION; got %+v", ds) }
}
...
func TestMerge_DedupFieldUnderPartition_Errors_WhenStrict(t *testing.T) {
    ...
    StrictDedupUnderPartition = false
    found := false
    for _, d := range ds { if d.Code == "E_MERGE_DEDUP_FIELD_WITHOUT_KEY_UNDER_PARTITION" { found = true; break } }
    if !found { t.Fatalf("expected E_MERGE_DEDUP_FIELD_WITHOUT_KEY_UNDER_PARTITION; got %+v", ds) }
}
