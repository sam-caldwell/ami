...
func TestMergeAttrTypes_Watermark_InvalidLateness(t *testing.T) {
    ...
    found := false
    for _, d := range ds { if d.Code == "E_MERGE_ATTR_TYPE" { found = true } }
    if !found { t.Fatalf("expected E_MERGE_ATTR_TYPE for invalid watermark lateness; got %+v", ds) }
}
...
func TestMergeAttrTypes_Window_NumberChecks(t *testing.T) {
    ...
    if !hasCodeRec(ds1, "E_MERGE_ATTR_TYPE") { t.Fatalf("expected E_MERGE_ATTR_TYPE; got %+v", ds1) }
    ...
    if !hasCodeRec(ds2, "W_MERGE_WINDOW_ZERO_OR_NEGATIVE") { t.Fatalf("expected W_MERGE_WINDOW_ZERO_OR_NEGATIVE; got %+v", ds2) }
}
...
func TestMergeAttrTypes_Timeout_NumberChecks(t *testing.T) {
    ...
    af2, _ := parser.New(f2).ParseFile()
    ds2 := AnalyzeMultiPath(af2)
    if !hasCodeRec(ds2, "E_MERGE_ATTR_ARGS") { t.Fatalf("expected E_MERGE_ATTR_ARGS; got %+v", ds2) }
}
