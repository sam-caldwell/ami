...
func TestAnalyzeMultiPath_Timeout_And_Watermark_Lateness(t *testing.T) {
    ...
    hasLateWarn := false
    for _, d := range ds {
        if d.Code == "E_MERGE_ATTR_ARGS" { hasTimeoutErr = true }
        if d.Code == "W_MERGE_WATERMARK_NONPOSITIVE" { hasLateWarn = true }
    }
    if !hasTimeoutErr || !hasLateWarn { t.Fatalf("expected timeout err and lateness warn: %+v", ds) }
}
...
func TestAnalyzeMultiPath_Dedup_Conflicts_With_Key(t *testing.T) {
    ...
    if !has { t.Fatalf("expected conflict between Dedup and Key: %+v", ds) }
}
