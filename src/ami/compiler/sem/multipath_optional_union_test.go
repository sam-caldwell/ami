...
func testMergeField_Optional_Struct_Unorderable(t *testing.T) {
	...
	found := false
	for _, d := range ds {
		if d.Code == "E_MERGE_SORT_FIELD_UNORDERABLE" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected unorderable for Optional<Struct>")
	}
}
...
func testMergeField_Union_Mixed_Unorderable(t *testing.T) {
	...
	if !found {
		t.Fatalf("expected unorderable for mixed union")
	}
}
...
func testMergeField_Union_MissingField_Unknown(t *testing.T) {
	...
	found := false
	for _, d := range ds {
		if d.Code == "E_MERGE_SORT_FIELD_UNKNOWN" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected unknown field for union missing member")
	}
}
