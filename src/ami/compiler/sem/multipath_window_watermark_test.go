...
func TestMerge_Window_Without_Watermark_Warns(t *testing.T) {
    ...
    found := false
    for _, d := range ds { if d.Code == "W_MERGE_WINDOW_WITHOUT_WATERMARK" { found = true; break } }
    if !found { t.Fatalf("expected W_MERGE_WINDOW_WITHOUT_WATERMARK; got %+v", ds) }
}
...
func TestMerge_Watermark_Not_Primary_Sort_Warns(t *testing.T) {
    ...
    ds := AnalyzeMultiPath(af)
    found := false
    for _, d := range ds { if d.Code == "W_MERGE_WATERMARK_NOT_PRIMARY_SORT" { found = true; break } }
    if !found { t.Fatalf("expected W_MERGE_WATERMARK_NOT_PRIMARY_SORT; got %+v", ds) }
}
