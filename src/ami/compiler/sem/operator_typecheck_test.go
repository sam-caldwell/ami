...
func f(a int, b string) { a + b }`
    ...
    res := AnalyzeFile(f)
    found := false
    for _, d := range res.Diagnostics { if d.Code == "E_TYPE_MISMATCH" { found = true; break } }
    if !found { t.Fatalf("expected E_TYPE_MISMATCH; diags=%v", res.Diagnostics) }
}
...
func f(a int, b int) { a + b }`
    ...
    f := p.ParseFile()
    res := AnalyzeFile(f)
    for _, d := range res.Diagnostics { if d.Code == "E_TYPE_MISMATCH" { t.Fatalf("unexpected mismatch: %v", d) } }
}
