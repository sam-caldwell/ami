...
func testEdgeCoverage_EdgesWithoutIngress(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "E_EDGES_WITHOUT_INGRESS" {
			has = true
			break
		}
	}
	if !has {
		t.Fatalf("expected E_EDGES_WITHOUT_INGRESS; got %+v", ds)
	}
}
...
func testEdgeCoverage_EdgesWithoutEgress(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "E_EDGES_WITHOUT_EGRESS" {
			has = true
			break
		}
	}
	if !has {
		t.Fatalf("expected E_EDGES_WITHOUT_EGRESS; got %+v", ds)
	}
}
