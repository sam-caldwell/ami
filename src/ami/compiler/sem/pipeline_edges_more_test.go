...
func testPipelineSemantics_SelfEdge_Error(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "E_PIPELINE_SELF_EDGE" {
			has = true
		}
	}
	if !has {
		t.Fatalf("expected E_PIPELINE_SELF_EDGE; got %+v", ds)
	}
}
...
func testPipelineSemantics_DuplicateEdge_Warn(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "W_PIPELINE_DUP_EDGE" {
			has = true
		}
	}
	if !has {
		t.Fatalf("expected W_PIPELINE_DUP_EDGE; got %+v", ds)
	}
}
