...
func TestPipelineSemantics_UnreachableFromIngress_WithDegree(t *testing.T) {
    ...
    found := false
    for _, d := range ds { if d.Code == "E_PIPELINE_UNREACHABLE_FROM_INGRESS" { found = true } }
    if !found { t.Fatalf("expected E_PIPELINE_UNREACHABLE_FROM_INGRESS, got %+v", ds) }
}
...
func TestPipelineSemantics_CannotReachEgress_WithDegree(t *testing.T) {
    ...
    ds := AnalyzePipelineSemantics(af)
    found := false
    for _, d := range ds { if d.Code == "E_PIPELINE_CANNOT_REACH_EGRESS" { found = true } }
    if !found { t.Fatalf("expected E_PIPELINE_CANNOT_REACH_EGRESS, got %+v", ds) }
}
