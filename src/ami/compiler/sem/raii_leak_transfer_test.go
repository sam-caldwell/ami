...
func TestRAII_Leak_OnOwnedResult_NotReleased(t *testing.T) {
    ...
    ds := AnalyzeRAII(af)
    has := false
    for _, d := range ds { if d.Code == "E_RAII_LEAK" { has = true } }
    if !has { t.Fatalf("expected E_RAII_LEAK, got: %+v", ds) }
}
...
func TestRAII_Transfer_Unowned_Error(t *testing.T) {
    ...
    ds := AnalyzeRAII(af)
    has := false
    for _, d := range ds { if d.Code == "E_RAII_TRANSFER_UNOWNED" { has = true } }
    if !has { t.Fatalf("expected E_RAII_TRANSFER_UNOWNED, got: %+v", ds) }
}
...
func TestRAII_NoLeak_WhenReleased(t *testing.T) {
    ...
    af, _ := p.ParseFileCollect()
    ds := AnalyzeRAII(af)
    for _, d := range ds { if d.Code == "E_RAII_LEAK" { t.Fatalf("unexpected leak: %+v", ds) } }
}
