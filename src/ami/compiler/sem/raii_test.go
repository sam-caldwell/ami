...
func testRAII_DeferRelease_NoDouble(t *testing.T) {
	...
	for _, d := range ds {
		if d.Code == "E_RAII_DOUBLE_RELEASE" {
			t.Fatalf("unexpected double-release: %+v", ds)
		}
	}
}
...
func testRAII_DoubleRelease_ImmediateThenDefer(t *testing.T) {
	...
	if !saw {
		t.Fatalf("expected E_RAII_DOUBLE_RELEASE, got: %+v", ds)
	}
}
...
func testRAII_UseAfterRelease_Immediate(t *testing.T) {
	...
	if !saw {
		t.Fatalf("expected E_RAII_USE_AFTER_RELEASE, got: %+v", ds)
	}
}
...
func testRAII_Release_Unowned(t *testing.T) {
	...
	saw := false
	for _, d := range ds {
		if d.Code == "E_RAII_RELEASE_UNOWNED" {
			saw = true
		}
	}
	if !saw {
		t.Fatalf("expected E_RAII_RELEASE_UNOWNED, got: %+v", ds)
	}
}
