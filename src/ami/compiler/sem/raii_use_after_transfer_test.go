...
func testRAII_UseAfterTransfer_NextStmt(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "E_RAII_USE_AFTER_TRANSFER" {
			has = true
		}
	}
	if !has {
		t.Fatalf("expected E_RAII_USE_AFTER_TRANSFER, got: %+v", ds)
	}
}
...
func testRAII_ReleaseAfterTransfer(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "E_RAII_RELEASE_AFTER_TRANSFER" {
			has = true
		}
	}
	if !has {
		t.Fatalf("expected E_RAII_RELEASE_AFTER_TRANSFER for release after transfer, got: %+v", ds)
	}
}
...
func testRAII_AssignAfterTransfer(t *testing.T) {
	...
	has := false
	for _, d := range ds {
		if d.Code == "E_RAII_ASSIGN_AFTER_TRANSFER" {
			has = true
		}
	}
	if !has {
		t.Fatalf("expected E_RAII_ASSIGN_AFTER_TRANSFER, got: %+v", ds)
	}
}
