...
func testReturn_Optional_StructFieldPath_ArityMismatch(t *testing.T) {
	...
	if !found {
		t.Fatalf("expected fieldPath a->b for Optional nested struct; got %+v", ds)
	}
}
...
func testReturn_Union_StructFieldPath_ArityMismatch(t *testing.T) {
	...
	for _, d := range ds {
		if d.Code == "E_GENERIC_ARITY_MISMATCH" && d.Data != nil {
			...
			okPath := false
			if pth, ok := d.Data["path"].([]string); ok {
				if len(pth) >= 1 && pth[len(pth)-1] == "Owned" {
					okPath = true
				}
			}
			if okFP && okPath {
	...
	if !found {
		t.Fatalf("expected fieldPath a->b and path ending with Owned for Union; got %+v", ds)
	}
}
