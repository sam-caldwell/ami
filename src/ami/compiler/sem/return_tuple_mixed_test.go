...
func TestReturnTypesWithSigs_MultiResult_DoesNotFitArity(t *testing.T) {
    ...
    ds := AnalyzeReturnTypesWithSigs(af, results)
    found := false
    for _, d := range ds { if d.Code == "E_RETURN_TYPE_MISMATCH" { found = true } }
    if !found { t.Fatalf("expected E_RETURN_TYPE_MISMATCH for arity; got %+v", ds) }
}
