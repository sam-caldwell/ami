...
func f(a int, b string) (int, string) { return a, b }`
    ...
    for _, d := range res.Diagnostics {
        if d.Code == "E_RETURN_TYPE_MISMATCH" { t.Fatalf("unexpected mismatch: %v", d) }
    }
}
...
func f(a int, b string) (int, string) { return a }`
    ...
    res := AnalyzeFile(f)
    found := false
    for _, d := range res.Diagnostics { if d.Code == "E_RETURN_TYPE_MISMATCH" { found = true; break } }
    if !found { t.Fatalf("expected E_RETURN_TYPE_MISMATCH; diags=%v", res.Diagnostics) }
}
