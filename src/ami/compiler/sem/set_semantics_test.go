...
`
	...
	found := false
	for _, d := range res.Diagnostics {
		if d.Code == "E_SET_ARITY" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_SET_ARITY; diags=%v", res.Diagnostics)
	}
}
...
`
	...
	found := false
	for _, d := range res.Diagnostics {
		if d.Code == "E_SET_ELEM_TYPE_INVALID" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_SET_ELEM_TYPE_INVALID for slice element; diags=%v", res.Diagnostics)
	}
}
...
`
	...
	if !found {
		t.Fatalf("expected E_SET_ELEM_TYPE_INVALID for map element; diags=%v", res.Diagnostics)
	}
}
...
`
	...
	if !found {
		t.Fatalf("expected E_SET_ELEM_TYPE_INVALID for pointer element; diags=%v", res.Diagnostics)
	}
}
