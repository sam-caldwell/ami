...
pipeline P { Ingress(cfg).Transform(f).Egress(cfg) }`
    ...
    for _, d := range res.Diagnostics {
        if d.Code == "E_WORKER_UNDEFINED" { t.Fatalf("unexpected undefined worker: %+v", d) }
    }
}
...
pipeline P { Ingress(cfg).Transform(doesNotExist()).Egress(cfg) }`
    ...
    res := AnalyzeFile(f)
    var seen bool
    for _, d := range res.Diagnostics { if d.Code == "E_WORKER_UNDEFINED" { seen = true; break } }
    if !seen { t.Fatalf("expected E_WORKER_UNDEFINED; got %+v", res.Diagnostics) }
}
...
pipeline P { Ingress(cfg).Transform(f).Egress(cfg) }`
    ...
    if !seen { t.Fatalf("expected E_WORKER_SIGNATURE; got %+v", res.Diagnostics) }
}
