...
func FromAST(tr astpkg.TypeRef) Type {
    ...
    switch base {
    case "Event":
        if len(tr.Args) == 1 { return EventType{Elem: FromAST(tr.Args[0])} }
        return TInvalid
    case "Error":
    ...
    case "map":
        ...
        return TInvalid
    case "set":
        if len(tr.Args) == 1 { return Set{Elem: FromAST(tr.Args[0])} }
        return TInvalid
    case "slice":
        if len(tr.Args) == 1 { return SliceTy{Elem: FromAST(tr.Args[0])} }
        return TInvalid
    }
    // Basic types (subset)
    switch base {
    ...
    // Named types
    t := Basic{name: base}
    if tr.Ptr { return Pointer{Base: t} }
    if tr.Slice { return Slice{Elem: t} }
