...
func TestParseOptionalUnion_StringRoundtrip(t *testing.T) {
    ...
    if err != nil { t.Fatalf("parse optional: %v", err) }
    if ty.String() != "Optional<int>" { t.Fatalf("roundtrip optional: %s", ty.String()) }
    tu, err := Parse("Union<int,string>")
    if err != nil { t.Fatalf("parse union: %v", err) }
...
func TestEqual_Structural_UnionOrderInsensitive(t *testing.T) {
    ...
    if !Equal(a, b) { t.Fatalf("expected union equality order-insensitive") }
}
...
func TestResolveField_Deep_WithOptionalUnion(t *testing.T) {
    ...
    if err != nil { t.Fatalf("parse root: %v", err) }
    leaf, ok := ResolveField(root, "x.y")
    if !ok { t.Fatalf("failed to resolve x.y") }
    // Expect Optional<Union<int,string>>
    want, _ := Parse("Optional<Union<int,string>>")
    if !Equal(leaf, want) { t.Fatalf("got %s want %s", leaf.String(), want.String()) }
    if !IsOrderable(leaf) { t.Fatalf("expected Optional<Union<int,string>> to be orderable") }
}

func TestResolveField_UnorderableLeaf(t *testing.T) {
    root, _ := Parse("Event<Struct{k:Struct{a:int}}>")
    leaf, ok := ResolveField(root, "k")
    if !ok { t.Fatalf("resolve k") }
    if IsOrderable(leaf) { t.Fatalf("struct leaf should be unorderable") }
}
