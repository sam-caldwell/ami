...
func New(opts Options) (*Logger, error) {
    ...
    if opts.Verbose {
        ...
        if err := os.MkdirAll(debugDir, 0o755); err == nil { // best-effort; ignore errors here
            f, err := os.OpenFile(filepath.Join(debugDir, "activity.log"), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
            if err == nil {
                l.alsoWrite = f
            }
        }
...
func (l *Logger) Close() error {
    if c, ok := l.alsoWrite.(io.Closer); ok && c != nil {
        return c.Close()
    }
    return nil
}
