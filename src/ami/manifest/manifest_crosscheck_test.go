...
func TestManifest_CrossCheckWithSumFile(t *testing.T) {
    ...
    if err := Save(manPath, m); err != nil { t.Fatalf("save manifest: %v", err) }
    ...
    if err := os.WriteFile(sumPath, b, 0o644); err != nil { t.Fatalf("write sum: %v", err) }

    got, err := Load(manPath)
    if err != nil { t.Fatalf("load: %v", err) }
    if err := got.CrossCheckWithSumFile(sumPath); err != nil { t.Fatalf("cross-check: %v", err) }
}
...
func TestManifest_CrossCheckWithSumFile_Mismatch(t *testing.T) {
    ...
    got, err := Load(manPath)
    if err != nil { t.Fatalf("load: %v", err) }
    if err := got.CrossCheckWithSumFile(sumPath); err == nil { t.Fatalf("expected cross-check mismatch error") }
}
