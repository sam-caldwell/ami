...
func loadSum(path string) (*Sum, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return &Sum{Schema: "ami.sum/v1", Packages: map[string]map[string]string{}}, nil
		}
		return nil, err
	}
	var s Sum
	...
	if s.Packages == nil {
		s.Packages = map[string]map[string]string{}
	}
	return &s, nil
}
...
func saveSum(path string, s *Sum) error {
	...
	for i, name := range pkgs {
		...
		for j, v := range vers {
			...
			b.WriteString(v)
			b.WriteString("\": \"")
			b.WriteString(s.Packages[name][v])
			b.WriteString("\"")
		}
		if len(vers) > 0 {
			b.WriteString("\n    }")
...
func UpdateSum(sumPath, pkg, version, repoPath, tag string) error {
	...
	if s.Packages[pkg] == nil {
		s.Packages[pkg] = map[string]string{}
	}
	...
	for _, d := range deps {
		...
		if strings.HasPrefix(repoPath, "./") || strings.HasPrefix(repoPath, "../") || strings.HasPrefix(repoPath, "/") {
			...
			if !strings.HasPrefix(tag, "v") {
				return fmt.Errorf("invalid or unsupported version: %s", tag)
			}
			cache, err := CacheDir()
			if err != nil {
				return err
			...
			_ = os.RemoveAll(dest)
			if err := copyDir(repoPath, dest); err != nil {
				return err
			}
			// Use repoPath for ami.sum key; verify uses base name to resolve cache entry
			if err := UpdateSum("ami.sum", repoPath, tag, dest, tag); err != nil {
				return err
			}
			continue
		}
		...
		if err := UpdateSum("ami.sum", pkg, tag, dest, tag); err != nil {
			return err
		}
	}
	return nil
...
func semverLess(a, b string) bool {
	...
	if apre == "" && bpre != "" {
		return false
	} // a is release, not less
	if apre != "" && bpre == "" {
		return true
	} // a is prerelease, less than release
...
func resolveConstraint(sshURL, cons string) (string, error) {
	...
	case strings.HasPrefix(cons, "^"):
		...
		for _, t := range tags {
			tv := parseSemVer(t)
			if tv[0] == bv[0] && !semverLess(t, base) {
				filt = append(filt, t)
			}
		}
	case strings.HasPrefix(cons, "~"):
		...
		for _, t := range tags {
			tv := parseSemVer(t)
			if tv[0] == bv[0] && tv[1] == bv[1] && !semverLess(t, base) {
				filt = append(filt, t)
			}
		}
	default:
	...
	if len(filt) == 0 {
		return "", errors.New("no matching tags for constraint")
	}
