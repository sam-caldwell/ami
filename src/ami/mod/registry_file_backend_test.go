...
func TestFileBackend_Fetch_LocalPath(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(proj, "README.md"), []byte("ok"), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	// ami.workspace with declared import
	wsFile := "packages:\n  import:\n    - ./example\n"
	if err := os.WriteFile(filepath.Join(ws, "ami.workspace"), []byte(wsFile), 0o644); err != nil {
	...
	if pkg != "" || ver != "" {
		t.Fatalf("file backend should not return pkg/ver; got %q %q", pkg, ver)
	}
