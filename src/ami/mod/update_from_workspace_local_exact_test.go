...
func TestUpdateFromWorkspace_LocalRepo_ExactVersion(t *testing.T) {
    ...
    if err := os.WriteFile(filepath.Join(repo, "f.txt"), []byte("x"), 0o644); err != nil { t.Fatalf("write: %v", err) }
    w, _ := r.Worktree()
    _, _ = w.Add("f.txt")
    ...
    if err := UpdateFromWorkspace(filepath.Join(ws, "ami.workspace")); err != nil { t.Fatalf("update: %v", err) }
    if _, err := os.Stat(filepath.Join(cacheDir, "absrepo@v1.0.0")); err != nil { t.Fatalf("expected cache entry absrepo@v1.0.0: %v", err) }
}
