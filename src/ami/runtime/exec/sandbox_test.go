...
func TestSandbox_TimerDenied(t *testing.T) {
    ...
    _, _, err = eng.RunPipelineWithStats(ctx, m, "P", in, nil, "none", "none", ExecOptions{SourceType: "timer", TimerInterval: 5 * time.Millisecond, TimerCount: 1, Sandbox: SandboxPolicy{AllowDevice: false, AllowFS: true, AllowNet: true}})
...
func TestSandbox_TimerAllowed(t *testing.T) {
    ...
    defer cancel()
    out, statsCh, err := eng.RunPipelineWithStats(ctx, m, "P", in, nil, "none", "none", ExecOptions{SourceType: "timer", TimerInterval: 5 * time.Millisecond, TimerCount: 3, Sandbox: SandboxPolicy{AllowDevice: true}})
