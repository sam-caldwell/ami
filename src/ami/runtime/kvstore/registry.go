...
func ResetDefault(opts ...Options) {
    defaultMu.Lock(); defer defaultMu.Unlock()
    if defaultReg != nil { defaultReg.Close() }
    var o Options
    if len(opts) > 0 { o = opts[0] } else { o = Options{CapacityBytes: 0, SweepInterval: 100 * time.Millisecond} }
...
func (r *Registry) Snapshot() []StoreInfo {
    ...
    for k, s := range r.table {
        ...
        for i := 0; i < len(k); i++ {
            if k[i] == '\x1f' { pipeline, node = k[:i], k[i+1:]; break }
        }
        if node == "" { pipeline = k }
