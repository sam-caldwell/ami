...
func TestStore_CapacityEvictsLRU(t *testing.T) {
    ...
    if _, ok := s.Get("k1"); !ok { t.Fatalf("expected k1 present") }
    s.Put("k3", "abcdefg")  // insert new -> should evict LRU (k2)

    if s.Has("k2") { t.Fatalf("expected k2 evicted") }
    if !s.Has("k1") || !s.Has("k3") { t.Fatalf("expected k1 and k3 remain") }
    m := s.Metrics()
    if m.Evictions == 0 { t.Fatalf("expected evictions > 0; got %+v", m) }
}
