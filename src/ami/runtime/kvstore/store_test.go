...
func testStore_PutGet_DeleteOnRead(t *testing.T) {
	...
	if v, ok := s.Get("k"); !ok || v.(int) != 123 {
		t.Fatalf("get1: %v %v", v, ok)
	}
	if _, ok := s.Get("k"); !ok {
		t.Fatalf("get2 missing")
	}
	if _, ok := s.Get("k"); ok {
		t.Fatalf("expected deletion after 2 reads")
	}
}
...
func testStore_TTL_Expiry(t *testing.T) {
	...
	if _, ok := s.Get("k"); !ok {
		t.Fatalf("unexpected miss before expiry")
	}
	...
	m := s.Metrics()
	if m.Expirations == 0 {
		t.Fatalf("expected expiration metric > 0")
	}
}
...
func testStore_SlidingTTL_ExtendsLifetime(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if _, ok := s.Get("k"); !ok {
			t.Fatalf("key expired early at i=%d", i)
		}
	}
	time.Sleep(20 * time.Millisecond)
	if _, ok := s.Get("k"); ok {
		t.Fatalf("expected expired after idle period")
	}
}
...
func testStore_CapacityLRU_EvictsOldest(t *testing.T) {
	...
	s.Put("d", 4)
	if _, ok := s.Get("c"); ok {
		t.Fatalf("expected 'c' evicted after touching 'b'")
	}
	m := s.Metrics()
	if m.Evictions < 2 {
		t.Fatalf("expected evictions>=2, got %d", m.Evictions)
	}
}
