...
func TestStore_TTL_AbsoluteExpiry(t *testing.T) {
	...
	if _, ok := s.Get("a"); !ok {
		t.Fatalf("expected a present initially")
	}
	...
	m := s.Metrics()
	if m.Expirations == 0 {
		t.Fatalf("expected expirations > 0; got %+v", m)
	}
}
...
func TestStore_TTL_SlidingRefresh(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		if _, ok := s.Get("b"); !ok {
			t.Fatalf("expected b present during sliding window")
		}
	...
	if _, ok := s.Get("b"); ok {
		t.Fatalf("expected b expired after idle period")
	}
}
