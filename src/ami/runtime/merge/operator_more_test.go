...
func testMerge_Window_CapsBuffer(t *testing.T) {
	...
	if !(k1 <= k2) {
		t.Fatalf("unsorted within window: %d %d", k1, k2)
	}
	if _, ok := op.Pop(); ok {
		t.Fatalf("expected only 2 items due to window")
	}
}
...
func testMerge_Watermark_DropsLate(t *testing.T) {
	...
	if _, ok := op.Pop(); ok {
		t.Fatalf("expected late event to be dropped")
	}
}
...
func testMerge_Partition_RoundRobin(t *testing.T) {
	...
	if p1 == p2 {
		t.Fatalf("not round-robin: %s %s %s", p1, p2, p3)
	}
}
...
func testMerge_Partition_SortedWithinEach(t *testing.T) {
	...
	for {
		...
		if m["p"].(string) == "x" {
			xs = append(xs, m["k"].(int))
		} else {
			ys = append(ys, m["k"].(int))
		}
	}
	if !(len(xs) == 2 && xs[0] <= xs[1]) {
		t.Fatalf("x not sorted: %v", xs)
	}
	if !(len(ys) == 2 && ys[0] <= ys[1]) {
		t.Fatalf("y not sorted: %v", ys)
	}
}
...
func testMerge_Desc_Float_Partitioned(t *testing.T) {
	...
	for {
		...
		m := e.Payload.(map[string]any)
		if m["p"].(string) == "x" {
			xs = append(xs, m["v"].(float64))
		} else {
			ys = append(ys, m["v"].(float64))
		}
	}
	if !(len(xs) == 2 && xs[0] >= xs[1]) {
		t.Fatalf("x desc wrong: %v", xs)
	}
	if !(len(ys) == 2 && ys[0] >= ys[1]) {
		t.Fatalf("y desc wrong: %v", ys)
	}
}
