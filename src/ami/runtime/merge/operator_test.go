...
func TestMerge_Sort_Stable(t *testing.T) {
    ...
    if err := op.Push(e1); err != nil { t.Fatal(err) }
    if err := op.Push(e2); err != nil { t.Fatal(err) }
    o1, ok := op.Pop(); if !ok { t.Fatal("no pop1") }
    o2, ok := op.Pop(); if !ok { t.Fatal("no pop2") }
...
func TestMerge_Dedup(t *testing.T) {
    ...
    _ = op.Push(e1)
    _ = op.Push(e2)
    if _, ok := op.Pop(); !ok { t.Fatal("expected first") }
    if _, ok := op.Pop(); ok { t.Fatal("expected dedup drop") }
}
