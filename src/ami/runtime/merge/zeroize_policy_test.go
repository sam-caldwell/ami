...
func testZeroization_OnDropNewest(t *testing.T) {
	...
	if err := op.Push(ev.Event{Payload: a}); err != nil {
		t.Fatalf("push a: %v", err)
	}
	if err := op.Push(ev.Event{Payload: b}); err != nil {
	...
	for i := range bb {
		if bb[i] != 0 {
			t.Fatalf("expected zeroized b; got %v", bb)
		}
	}
	// a remains in buffer, not zeroized
	ab := a["secret"].([]byte)
	if ab[0] == 0 && len(ab) > 0 {
		t.Fatalf("unexpected zeroization of retained payload: %v", ab)
	}
}
...
func testZeroization_OnDropOldest(t *testing.T) {
	...
	for i := range ab {
		if ab[i] != 0 {
			t.Fatalf("expected zeroized a; got %v", ab)
		}
	}
	bb := b["secret"].([]byte)
	if bb[0] == 0 && len(bb) > 0 {
		t.Fatalf("unexpected zeroization of retained payload: %v", bb)
	}
}
