...
func WithKV(pipeline, node string) KVOpt { return func(c *KVConfig){ c.Pipeline, c.Node = pipeline, node } }
...
func BuildKVInput(payload map[string]any, opts ...KVOpt) (string, error) {
    ...
    if c.Pipeline != "" { m["kv_pipeline"] = c.Pipeline }
    if c.Node != "" { m["kv_node"] = c.Node }
    if c.PutKey != "" { m["kv_put_key"] = c.PutKey; m["kv_put_val"] = c.PutVal }
    if c.GetKey != "" { m["kv_get_key"] = c.GetKey }
    if c.Emit { m["kv_emit"] = true }
