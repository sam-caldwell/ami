...
func TestBufio_Writer_FlushSemantics(t *testing.T) {
	...
	if underlying.Len() != 0 {
		t.Fatalf("unexpected write-through before flush: %d", underlying.Len())
	}
	if err := w.Flush(); err != nil {
		t.Fatal(err)
	}
	if underlying.String() != "hello" {
		t.Fatalf("flush got %q", underlying.String())
	}
}
...
func TestBufio_Writer_BufferFull_PartialFlush(t *testing.T) {
	...
	if underlying.Len() != 0 {
		t.Fatalf("unexpected write-through before full: %q", underlying.String())
	}
	if _, err := w.Write([]byte("c")); err != nil {
		t.Fatal(err)
	}
	if underlying.Len() != 0 {
		t.Fatalf("unexpected flush at exactly full: %q", underlying.String())
	}
	if _, err := w.Write([]byte("d")); err != nil {
		t.Fatal(err)
	}
	if underlying.String() != "abc" {
		t.Fatalf("expected flush when exceeding buffer, got %q", underlying.String())
	}
	...
	if underlying.String() != "abcde" {
		t.Fatalf("after flush got %q", underlying.String())
	}
}
...
func TestBufio_InvalidSizes(t *testing.T) {
	if _, err := NewReaderSize(stdbytes.NewBuffer(nil), 0); err == nil {
		t.Fatal("expected error")
	}
	var b stdbytes.Buffer
	if _, err := NewWriterSize(&b, -1); err == nil {
		t.Fatal("expected error")
	}
}
