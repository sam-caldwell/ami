...
func TestSHA256_Hasher_StreamVsOneShot(t *testing.T) {
    ...
    for _, ch := range [][]byte{data[:10], data[10:20], data[20:]} {
        if _, err := h.Write(ch); err != nil { t.Fatal(err) }
    }
    two := h.Sum()
    if one != two { t.Fatal("streaming hash mismatch") }

    // Reset and write again
    h.Reset()
    if _, err := h.Write(data); err != nil { t.Fatal(err) }
    if h.Sum() != one { t.Fatal("reset+write mismatch") }
}
