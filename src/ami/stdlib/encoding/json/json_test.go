...
func TestMarshal_StableAcrossRuns(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		if !bytes.Equal(got, first) {
			t.Fatal("non-deterministic marshal across runs")
		}
	}
}
...
func TestMarshal_ScalarsAndNull(t *testing.T) {
	...
	if err != nil || string(b) != "null" {
		t.Fatalf("nil got %s err %v", b, err)
	}
	// bool
	b, err = Marshal(true)
	if err != nil || string(b) != "true" {
		t.Fatalf("bool got %s err %v", b, err)
	}
	// string
	b, err = Marshal("hi")
	if err != nil || string(b) != "\"hi\"" {
		t.Fatalf("string got %s err %v", b, err)
	}
}
...
func TestUnmarshal_UseNumber(t *testing.T) {
	var m map[string]any
	if err := Unmarshal([]byte(`{"n": 1}`), &m); err != nil {
		t.Fatal(err)
	}
	if _, ok := m["n"].(stdjson.Number); !ok {
		t.Fatalf("number type not preserved: %T", m["n"])
	}
}
...
func TestCanonicalize_And_Compact(t *testing.T) {
	...
	if string(can) != `{"a":{"y":3,"z":2},"b":1}` {
		t.Fatalf("canonical got %s", can)
	}
	cmp, err := Compact(can)
	if err != nil {
		t.Fatal(err)
	}
	if string(cmp) != string(can) {
		t.Fatalf("compact changed bytes: %s vs %s", cmp, can)
	}
}
...
func TestEqualCanonical_And_Valid(t *testing.T) {
	...
	if !eq {
		t.Fatal("expected canonical equality")
	}
	if !Valid(a) || !Valid(b) {
...
func TestEqualCanonical_ErrorOnInvalid(t *testing.T) {
	if _, err := EqualCanonical([]byte(`}`), []byte(`{}`)); err == nil {
		t.Fatal("expected error")
	}
}

func TestCompact_ErrorOnInvalid(t *testing.T) {
	if _, err := Compact([]byte(`{"a":}`)); err == nil {
		t.Fatal("expected error")
	}
}
