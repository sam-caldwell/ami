...
func TestIO_CopyN_Happy(t *testing.T) {
    ...
    if n != 5 || dst.String() != "hello" { t.Fatalf("copied %d %q", n, dst.String()) }
}
...
func TestIO_CopyN_ShortSrc_Sad(t *testing.T) {
    ...
    var dst stdbytes.Buffer
    n, err := CopyN(&dst, src, 5)
    if err == nil { t.Fatal("expected error due to short read") }
    if n != 2 || dst.String() != "hi" { t.Fatalf("copied %d %q", n, dst.String()) }
}
