...
func TestCreateTemp_CreateTempDir_Stat_Name(t *testing.T) {
    ...
    if err != nil { t.Fatalf("CreateTemp failed: %v", err) }
    defer fho.Close()
    if n, err := fho.Write([]byte("abc")); err != nil || n != 3 { t.Fatalf("Write: n=%d err=%v", n, err) }
    if err := fho.Flush(); err != nil { t.Fatalf("Flush: %v", err) }

    name := fho.Name()
    if name == "" { t.Fatalf("Name() should not be empty") }
    if !strings.HasSuffix(name, ".log") { t.Fatalf("temp file suffix missing: %s", name) }

    info, err := Stat(name)
    if err != nil { t.Fatalf("Stat failed: %v", err) }
    if info.Size != 3 || info.IsDir { t.Fatalf("Stat unexpected: %+v", info) }

    // Temp dir
    dir, err := CreateTempDir()
    if err != nil { t.Fatalf("CreateTempDir failed: %v", err) }
    if fi, err := os.Stat(dir); err != nil || !fi.IsDir() { t.Fatalf("temp dir invalid: %v isDir=%v err=%v", dir, fi.IsDir(), err) }
}
...
func TestStdStreams_CloseOnlyMarksHandle(t *testing.T) {
    ...
    if n, err := Stdout.Write([]byte("")); err != nil || n != 0 { t.Fatalf("Stdout write zero bytes err=%v n=%d", err, n) }
    if n, err := Stderr.Write([]byte("")); err != nil || n != 0 { t.Fatalf("Stderr write zero bytes err=%v n=%d", err, n) }

    if err := Stdout.Close(); err != nil { t.Fatalf("Stdout close: %v", err) }
    if err := Stderr.Close(); err != nil { t.Fatalf("Stderr close: %v", err) }

    if _, err := Stdout.Write([]byte("x")); err == nil || err != ErrClosed { t.Fatalf("Stdout expected ErrClosed, got %v", err) }
    if _, err := Stderr.Write([]byte("x")); err == nil || err != ErrClosed { t.Fatalf("Stderr expected ErrClosed, got %v", err) }
}

func TestNameNilAndTempVariantsAndClosedTruncate(t *testing.T) {
    // Name on nil receiver returns empty
    var hnil *FHO
    if name := hnil.Name(); name != "" { t.Fatalf("nil Name() => %q, want empty", name) }

    // CreateTemp with only dir argument
    fho, err := CreateTemp("ami-tests2")
    if err != nil { t.Fatalf("CreateTemp(dir) failed: %v", err) }
    path := fho.Name()
    ...
    fho2, err := CreateTemp()
    if err != nil { t.Fatalf("CreateTemp failed: %v", err) }
    if err := fho2.Close(); err != nil { t.Fatalf("close: %v", err) }
