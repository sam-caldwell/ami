...
func testSocket_CloseReadCloseWrite_OnUDP_ReturnError(t *testing.T) {
	...
	if err := s.CloseRead(); err == nil {
		t.Fatalf("CloseRead should error on UDP")
	}
	if err := s.CloseWrite(); err == nil {
		t.Fatalf("CloseWrite should error on UDP")
	}
}
...
func testSocket_NoUnderlying_Errors(t *testing.T) {
	...
	if err := s.SetDeadline(time.Now()); err == nil {
		t.Fatalf("expected error SetDeadline")
	}
	if err := s.SetReadDeadline(time.Now()); err == nil {
		t.Fatalf("expected error SetReadDeadline")
	}
	if err := s.SetWriteDeadline(time.Now()); err == nil {
		t.Fatalf("expected error SetWriteDeadline")
	}
}
