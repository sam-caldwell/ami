...
func TestProcError_ErrorString(t *testing.T) {
    e := &procError{"x"}
    if e.Error() != "x" { t.Fatalf("procError.Error mismatch: %q", e.Error()) }
}

func TestExitCodeFromError(t *testing.T) {
    if exitCodeFromError(nil) != nil { t.Fatalf("nil error -> nil exit code") }
...
func TestPrestartWriter_AfterStart_Forwards(t *testing.T) {
    ...
    if _, err := w.Write([]byte("noop")); err != nil { t.Fatalf("write after start: %v", err) }
}
