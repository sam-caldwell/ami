...
func TestStart_Twice_ReturnsError(t *testing.T) {
    ...
    if err := p.Start(false); err == nil { t.Fatalf("expected error on second start") }
}

func TestKill_BeforeStart_ReturnsError(t *testing.T) {
    p, err := Exec("go", "version")
    if err != nil { t.Fatalf("exec: %v", err) }
    if err := p.Kill(); err == nil { t.Fatalf("expected kill error before start") }
}

func TestNilReceiversAndStdStreams(t *testing.T) {
    var p *Process
    if _, err := p.Stdin(); err == nil { t.Fatalf("expected error on nil.Stdin()") }
    if got := p.Stdout(); got != nil { t.Fatalf("nil.Stdout() should be nil, got %v", got) }
    if got := p.Stderr(); got != nil { t.Fatalf("nil.Stderr() should be nil, got %v", got) }
    if err := p.Start(false); err == nil { t.Fatalf("expected error on nil.Start()") }
}
...
func TestStatus_BeforeStart(t *testing.T) {
    ...
    if err != nil { t.Fatalf("exec: %v", err) }
    st := p.Status()
    if st.Running { t.Fatalf("expected not running before start") }
    if st.ExitCode != nil { t.Fatalf("expected nil exit code before start") }
}
...
func main(){ os.Exit(3) }`
    ...
    if err != nil { t.Fatalf("exec: %v", err) }
    err = p.Start(true)
    if err == nil { t.Fatalf("expected non-nil error from non-zero exit") }
    st := p.Status()
    if st.Running { t.Fatalf("should not be running after blocking run") }
    if st.ExitCode == nil || *st.ExitCode != 3 {
        got := 0
        if st.ExitCode != nil { got = *st.ExitCode }
...
func main(){ io.Copy(os.Stdout, os.Stdin) }`
    ...
    msg := "prestart-write\n"
    if _, err := w.Write([]byte(msg)); err != nil { t.Fatalf("write before start: %v", err) }
