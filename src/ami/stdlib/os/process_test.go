...
func testExecAndStart_Blocking_GoVersion(t *testing.T) {
	...
	if pid := p.Pid(); pid != 0 {
		t.Fatalf("pid should be 0 before start, got %d", pid)
	}
	...
	st := p.Status()
	if st.Running {
		t.Fatalf("expected not running after blocking run")
	}
...
func testExecAndStart_Async_StatusCompletes(t *testing.T) {
	...
	if w, err := p.Stdin(); err == nil {
		_ = w.Close()
	}
	if pid := p.Pid(); pid <= 0 {
		t.Fatalf("expected pid > 0 after start, got %d", pid)
	}
