...
func testFromOSSignal_Mapping(t *testing.T) {
	if fromOSSignal(os.Interrupt) != SIGINT {
		t.Fatalf("Interrupt -> SIGINT")
	}
	// These may not be present on all platforms; best-effort checks
	if fromOSSignal(syscall.SIGTERM) != SIGTERM {
		t.Fatalf("SIGTERM mapping")
	}
...
func testToOSSignal_AllCases(t *testing.T) {
	...
	if toOSSignal(SIGINT) == nil {
		t.Fatalf("SIGINT toOSSignal nil")
	}
	if toOSSignal(SIGTERM) == nil {
		t.Fatalf("SIGTERM toOSSignal nil")
	}
	if toOSSignal(SIGHUP) == nil {
		t.Fatalf("SIGHUP toOSSignal nil")
	}
	if toOSSignal(SIGQUIT) == nil {
		t.Fatalf("SIGQUIT toOSSignal nil")
	}
}
