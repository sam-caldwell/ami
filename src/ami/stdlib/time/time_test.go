...
func TestTime_FormatParseRFC3339Millis_RoundTrip(t *testing.T) {
	...
	if !back.Equal(t0.UTC().Truncate(stdtime.Millisecond)) {
		t.Fatalf("roundtrip mismatch: %v vs %v", back, t0)
	}
}
...
func TestTime_Duration_ParseFormat_RoundTrip(t *testing.T) {
	...
	if FormatDuration(d) != "1h2m3.004s" {
		t.Fatalf("format got %q", FormatDuration(d))
	}
	...
	if d2 != d {
		t.Fatal("duration roundtrip mismatch")
	}
}
...
func TestTime_Clock_Fixed(t *testing.T) {
	...
	c := FixedClock{T: now}
	if !c.Now().Equal(now) {
		t.Fatal("fixed clock mismatch")
	}
}

func TestTime_Sad_ParseError(t *testing.T) {
	if _, err := ParseRFC3339Millis("2025-09-25 12:00:00"); err == nil {
