...
func TestLinker_YAMLKeysPresent(t *testing.T) {
    ...
    if !strings.Contains(string(b), "options:") { t.Fatalf("expected options key; got: %s", string(b)) }
}
...
func TestLinter_YAMLKeysPresent(t *testing.T) {
    ...
    if err != nil { t.Fatalf("yaml: %v", err) }
    s := string(b)
    if !strings.Contains(s, "options:") { t.Fatalf("expected options key; got: %s", s) }
    if !strings.Contains(s, "rules:") { t.Fatalf("expected rules key; got: %s", s) }
}
