...
func testManifest_Load_ObjectForm(t *testing.T) {
  ...
  "packages": {
	...
	if m.Schema != "ami.sum/v1" {
		t.Fatalf("schema: %s", m.Schema)
	}
	if m.Packages["alpha"]["v1.0.0"] != "aaa" {
		t.Fatalf("alpha missing")
	}
	if m.Packages["beta"]["v2.0.0"] != "bbb" {
...
func testManifest_Save_RoundTrip_Nested(t *testing.T) {
	...
	var m2 Manifest
	if err := m2.Load(p); err != nil {
		t.Fatalf("Load2: %v", err)
	}
	if m2.Packages["alpha"]["v1.0.0"] != "aaa" || m2.Packages["alpha"]["v1.1.0"] != "aab" {
		t.Fatalf("alpha mismatch")
	}
	if m2.Packages["beta"]["v2.0.0"] != "bbb" {
...
func testManifest_Validate_UsesCacheAndHashes(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(p, "file.txt"), []byte("hi"), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	...
	if len(v) != 1 || v[0] != "gamma@v3.0.0" {
		t.Fatalf("verified mismatch: %+v", v)
	}
	if len(miss) != 1 || miss[0] != "delta@v4.0.0" {
		t.Fatalf("missing mismatch: %+v", miss)
	}
	if len(mm) != 0 {
		t.Fatalf("mismatched unexpected: %+v", mm)
	}
}
...
func testManifest_Helpers_SetHasVersions(t *testing.T) {
	var m Manifest
	if m.Has("x", "1.0.0") {
		t.Fatalf("unexpected Has on empty")
	}
