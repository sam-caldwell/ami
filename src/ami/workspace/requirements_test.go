...
func TestCollectRemoteRequirements_ParsesAndSkipsLocal(t *testing.T) {
    ...
    if len(errs) != 0 { t.Fatalf("unexpected errs: %v", errs) }
    if len(reqs) != 3 { t.Fatalf("want 3 reqs, got %d: %+v", len(reqs), reqs) }
...
func TestCrossCheckRequirements_MissingAndUnsatisfied(t *testing.T) {
    ...
    _ = os.RemoveAll(dir)
    if err := os.MkdirAll(filepath.Join(dir, "modA", "1.2.3"), 0o755); err != nil { t.Fatalf("mkdir: %v", err) }
    if err := os.WriteFile(filepath.Join(dir, "modA", "1.2.3", "x.txt"), []byte("hi"), 0o644); err != nil { t.Fatalf("write: %v", err) }
    ...
    if len(miss) != 1 || miss[0] != "modC" { t.Fatalf("missing: %v", miss) }
    if len(unsat) != 1 || unsat[0] != "modB" { t.Fatalf("unsatisfied: %v", unsat) }
}
