...
func testRunBuild_CrossPkg_LocalImport_MissingPath_JSON(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("expected error due to missing local import path")
	}
	...
	for s.Scan() {
		...
		if m["code"] == "E_IMPORT_LOCAL_MISSING" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_IMPORT_LOCAL_MISSING; out=\n%s", out.String())
	}
}
...
func testRunBuild_CrossPkg_LocalImport_Undeclared_JSON(t *testing.T) {
	...
	err := runBuild(&out, dir, true, false)
	if err == nil {
		t.Fatalf("expected error due to undeclared local import")
	}
	s := bufio.NewScanner(bytes.NewReader(out.Bytes()))
	found := false
	for s.Scan() {
		...
		if m["code"] == "E_IMPORT_LOCAL_UNDECLARED" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected E_IMPORT_LOCAL_UNDECLARED; out=\n%s", out.String())
	}
}
