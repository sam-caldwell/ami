...
// For darwin/amd64 env, ensure per-env .ll emission occurs and env obj dir exists.
func TestRunBuild_DarwinAmd64_EnvArtifacts(t *testing.T) {
    dir := filepath.Join("build", "test", "ami_build", "darwin_amd64_artifacts")
    _ = os.RemoveAll(dir)
    if err := os.MkdirAll(filepath.Join(dir, "src"), 0o755); err != nil { t.Fatalf("mkdir: %v", err) }
    ws := workspace.DefaultWorkspace()
    ws.Toolchain.Compiler.Env = []string{"darwin/amd64"}
    ...
    objDir := filepath.Join(dir, "build", "darwin", "amd64", "obj", ws.Packages[0].Package.Name)
