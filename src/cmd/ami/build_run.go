...
func runBuildImpl(out io.Writer, dir string, jsonOut bool, verbose bool) error {
	...
	if st, err := os.Stat(maniPath); err == nil && !st.IsDir() {
		...
		for name, vers := range sum.Packages {
			for ver := range vers {
				have[pair{name, ver}] = true
			}
		}
		for name, vers := range mani.Packages {
			for ver := range vers {
				want[pair{name, ver}] = true
			}
		}
	...
	if _, err := os.Stat(filepath.Join(dir, "ami.sum.sig")); err == nil {
		if err := verifySig(filepath.Join(dir, "ami.sum"), filepath.Join(dir, "ami.sum.sig")); err != nil {
			return err
		}
	}
	if _, err := os.Stat(filepath.Join(dir, "ami.manifest.sig")); err == nil {
		if err := verifySig(filepath.Join(dir, "ami.manifest"), filepath.Join(dir, "ami.manifest.sig")); err != nil {
			return err
		}
	}
		...
		if len(dbgPkgs) > 0 {
			...
			if lg := getRootLogger(); lg != nil {
				...
				if tool, err := be.FindToolchain(); err == nil {
					if ver, verr := be.ToolVersion(tool); verr == nil {
						lg.Info("toolchain.clang", map[string]any{"version": ver, "path": tool})
					}
				} else {
	...
	if !buildNoLink {
		buildLink(out, dir, ws, envs, jsonOut)
	}
	...
	// Rewrite ami.manifest into build/ami.manifest with toolchain metadata and objIndex entries.
	{
		...
		if len(rtObjs) > 0 {
			sort.Strings(rtObjs)
			outObj["runtimeObjects"] = rtObjs
		}
		if len(artifacts) > 0 {
			outObj["artifacts"] = artifacts
		}
		...
		if len(binsByEnv) > 0 {
			outObj["binariesByEnv"] = binsByEnv
		}
		...
		for _, env := range envs {
			...
			for _, e := range ws.Packages {
				...
				if st, err := os.Stat(idx); err == nil && !st.IsDir() {
					if rel, rerr := filepath.Rel(dir, idx); rerr == nil {
						objIndexByEnv[env] = append(objIndexByEnv[env], rel)
					}
				}
			}
			if len(objs) > 0 {
				sort.Strings(objs)
				objectsByEnv[env] = objs
			}
		}
		if len(objectsByEnv) > 0 {
			outObj["objectsByEnv"] = objectsByEnv
		}
		if len(objIndexByEnv) > 0 {
			outObj["objIndexByEnv"] = objIndexByEnv
		}
		if verbose {
			...
			for _, e := range ws.Packages {
				...
				if matches, _ := filepath.Glob(glob); len(matches) > 0 {
					for _, m := range matches {
						if rel, err := filepath.Rel(dir, m); err == nil {
							debugRefs = append(debugRefs, rel)
						}
					}
				}
				// IR
				...
				if st, err := os.Stat(idx); err == nil && !st.IsDir() {
					if rel, err := filepath.Rel(dir, idx); err == nil {
						debugRefs = append(debugRefs, rel)
					}
				}
			}
			if len(debugRefs) > 0 {
				sort.Strings(debugRefs)
				outObj["debug"] = debugRefs
			}
		}
	...
	if verbose {
		...
		for _, e := range ws.Packages {
			...
			if matches, _ := filepath.Glob(filepath.Join(dir, "build", "obj", e.Package.Name, "*.o")); len(matches) > 0 {
				for _, m := range matches {
					if rel, err := filepath.Rel(dir, m); err == nil {
						plan.Objects = append(plan.Objects, rel)
					}
				}
			}
			// debug IR indices
			...
			if st, err := os.Stat(irIdx); err == nil && !st.IsDir() {
				if rel, err := filepath.Rel(dir, irIdx); err == nil {
					plan.IRIndex = append(plan.IRIndex, rel)
				}
			}
			irTypes := filepath.Join(dir, "build", "debug", "ir", e.Package.Name, "ir.types.index.json")
			if st, err := os.Stat(irTypes); err == nil && !st.IsDir() {
				if rel, err := filepath.Rel(dir, irTypes); err == nil {
					plan.IRTypesIndex = append(plan.IRTypesIndex, rel)
				}
			}
			irSyms := filepath.Join(dir, "build", "debug", "ir", e.Package.Name, "ir.symbols.index.json")
			if st, err := os.Stat(irSyms); err == nil && !st.IsDir() {
				if rel, err := filepath.Rel(dir, irSyms); err == nil {
					plan.IRSymbolsIndex = append(plan.IRSymbolsIndex, rel)
				}
			}
		}
		if len(plan.Packages) > 1 {
			sort.Slice(plan.Packages, func(i, j int) bool {
				if plan.Packages[i].Name == plan.Packages[j].Name {
					return plan.Packages[i].Root < plan.Packages[j].Root
				}
				return plan.Packages[i].Name < plan.Packages[j].Name
			})
		}
		if len(plan.ObjIndex) > 1 {
			sort.Strings(plan.ObjIndex)
		}
		if len(plan.Objects) > 1 {
			sort.Strings(plan.Objects)
		}
		if len(plan.IRIndex) > 1 {
			sort.Strings(plan.IRIndex)
		}
		if len(plan.IRTypesIndex) > 1 {
			sort.Strings(plan.IRTypesIndex)
		}
		if len(plan.IRSymbolsIndex) > 1 {
			sort.Strings(plan.IRSymbolsIndex)
		}
		...
		for _, env := range envs {
			...
			if len(list) > 0 {
				sort.Strings(list)
				plan.ObjectsByEnv[env] = list
			}
			if v := plan.ObjIndexByEnv[env]; len(v) > 1 {
				sort.Strings(v)
				plan.ObjIndexByEnv[env] = v
			}
		}
		if len(plan.ObjectsByEnv) == 0 {
			plan.ObjectsByEnv = nil
		}
		if len(plan.ObjIndexByEnv) == 0 {
			plan.ObjIndexByEnv = nil
		}
		_ = os.MkdirAll(filepath.Join(dir, "build", "debug"), 0o755)
