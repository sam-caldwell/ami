...
func TestRunBuild_UsesWorkspaceConfig_JSON(t *testing.T) {
    ...
    if data == nil { t.Fatalf("expected data present") }
    if data["targetDir"] == "" { t.Fatalf("expected targetDir populated") }
    targets, ok := data["targets"].([]any)
    if !ok || len(targets) != 2 { t.Fatalf("expected 2 targets; got %v", data["targets"]) }
}
...
func TestRunBuild_DefaultEnvWhenEmpty_JSON(t *testing.T) {
    ...
    if len(targets) != 1 { t.Fatalf("expected default single target; got %v", targets) }
}
