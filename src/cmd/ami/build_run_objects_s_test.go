...
func TestRunBuild_NonVerbose_EmitsPerUnitAsm(t *testing.T) {
    dir := filepath.Join("build", "test", "ami_build", "nonverbose_obj_s")
    _ = os.RemoveAll(dir)
    ...
    p := filepath.Join(dir, "build", "obj", ws.Packages[0].Package.Name, "u.s")
    if _, err := os.Stat(p); err != nil { t.Fatalf("per-unit asm missing: %v", err) }
}
