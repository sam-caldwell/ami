...
func TestRunBuild_Verbose_PlanDeterministicAcrossRuns(t *testing.T) {
    ...
    // First run
    if err := runBuild(os.Stdout, dir, false, true); err != nil { t.Fatalf("run1: %v", err) }
    p := filepath.Join(dir, "build", "debug", "build.plan.json")
    b1, err := os.ReadFile(p)
    if err != nil { t.Fatalf("read1: %v", err) }

    // Second run (no changes)
    if err := runBuild(os.Stdout, dir, false, true); err != nil { t.Fatalf("run2: %v", err) }
...
func TestRunBuild_Verbose_PlanSortedCollections(t *testing.T) {
    ...
    if err := os.MkdirAll(filepath.Join(dir, "a"), 0o755); err != nil { t.Fatalf("mkdir a: %v", err) }
    if err := os.MkdirAll(filepath.Join(dir, "b"), 0o755); err != nil { t.Fatalf("mkdir b: %v", err) }
    if err := os.WriteFile(filepath.Join(dir, "a", "a1.ami"), []byte("package a\nfunc A(){}\n"), 0o644); err != nil { t.Fatalf("write a1: %v", err) }
    ...
    if err := runBuild(os.Stdout, dir, false, true); err != nil { t.Fatalf("run: %v", err) }
    p := filepath.Join(dir, "build", "debug", "build.plan.json")
    b, err := os.ReadFile(p)
    ...
    if e := json.Unmarshal(b, &plan); e != nil { t.Fatalf("json: %v", e) }
    if len(plan.Packages) != 2 { t.Fatalf("packages len=%d", len(plan.Packages)) }
    ...
    for _, s := range plan.Objects {
        if strings.Contains(s, "/a/") { if seenB { t.Fatalf("objects not sorted: %v", plan.Objects) }; seenA = true }
        if strings.Contains(s, "/b/") { seenB = true }
    }
    if !(seenA && seenB) { t.Fatalf("expected objects for both a and b: %v", plan.Objects) }
    // ObjIndex should list a before b
    if len(plan.ObjIndex) != 2 { t.Fatalf("objIndex len=%d", len(plan.ObjIndex)) }
