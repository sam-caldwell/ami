...
func TestRunBuild_Signature_Verify_Sum_Happy(t *testing.T) {
    ...
    h := sha256.Sum256(sum)
    if err := os.WriteFile(filepath.Join(dir, "ami.sum.sig"), []byte(hex.EncodeToString(h[:])), 0o644); err != nil { t.Fatalf("write sig: %v", err) }
...
func TestRunBuild_Signature_Verify_Sum_Sad(t *testing.T) {
    ...
    if err := os.WriteFile(filepath.Join(dir, "ami.sum"), sum, 0o644); err != nil { t.Fatalf("write sum: %v", err) }
    // wrong signature
    if err := os.WriteFile(filepath.Join(dir, "ami.sum.sig"), []byte("deadbeef"), 0o644); err != nil { t.Fatalf("write sig: %v", err) }
    if err := runBuild(os.Stdout, dir, true, false); err == nil { t.Fatalf("expected signature failure") }
}
