...
func TestLint_CrossPackage_RangeIntersection_EmptyAtBoundary(t *testing.T) {
    ...
    if !sawConflict { t.Fatalf("expected E_IMPORT_CONSTRAINT; out=%s", buf.String()) }
    if sawSingle { t.Fatalf("did not expect W_IMPORT_SINGLE_VERSION alongside hard conflict; out=%s", buf.String()) }
}
...
func TestLint_CrossPackage_RangeIntersection_Overlapping(t *testing.T) {
    ...
    if sawConflict { t.Fatalf("did not expect E_IMPORT_CONSTRAINT; out=%s", buf.String()) }
    if !sawSingle { t.Fatalf("expected W_IMPORT_SINGLE_VERSION when only ranges present; out=%s", buf.String()) }
}
