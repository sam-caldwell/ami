...
func TestLint_MemorySafety_EmitsDiagnostics_JSON(t *testing.T) {
    ...
    for dec.More() {
        ...
        if m["code"] == "E_PTR_UNSUPPORTED_SYNTAX" { sawPtr = true }
        if m["code"] == "E_MUT_BLOCK_UNSUPPORTED" { sawMut = true }
    }
    if !sawPtr || !sawMut { t.Fatalf("expected memory safety diagnostics; out=%s", buf.String()) }
}
...
func TestLint_MemorySafety_PragmaDisable(t *testing.T) {
    ...
    for dec.More() {
        var m map[string]any
        if derr := dec.Decode(&m); derr != nil { t.Fatalf("json: %v", derr) }
        if m["code"] == "E_PTR_UNSUPPORTED_SYNTAX" { t.Fatalf("pragma failed to suppress ptr diag: %s", buf.String()) }
    }
}
