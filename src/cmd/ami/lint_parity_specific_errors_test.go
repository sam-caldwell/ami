...
func testLint_Parity_Errors_IOPermission(t *testing.T) {
	...
	// JSON
	var js bytes.Buffer
	if err := runLint(&js, dir, true, false, false); err == nil {
		t.Fatalf("expected error; out=%s", js.String())
	}
	...
	if !sawCode || errors == 0 {
		t.Fatalf("expected E_IO_PERMISSION and errors>0; out=%s", js.String())
	}

	// Human
	var hm bytes.Buffer
	if err := runLint(&hm, dir, false, false, false); err == nil {
		t.Fatalf("expected error; out=%s", hm.String())
	}
	s := hm.String()
	if !regexp.MustCompile(`lint: \d+ error\(s\), \d+ warning\(s\)`).MatchString(s) {
		t.Fatalf("missing human summary: %s", s)
	}
	// Ensure position formatting present
	if !regexp.MustCompile(`\(.*:\d+:\d+\)`).MatchString(s) {
		t.Fatalf("missing (file:line:col) formatting: %s", s)
	}
}
...
func testLint_Parity_Strict_DupFunc(t *testing.T) {
	...
	f1 := "package x\nfunc F(){}\n"
	f2 := "package x\nfunc F(){}\n"
	if err := os.WriteFile(filepath.Join(srcDir, "a.ami"), []byte(f1), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	if err := os.WriteFile(filepath.Join(srcDir, "b.ami"), []byte(f2), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	ws := workspace.DefaultWorkspace()
	ws.Packages[0].Package.Root = "./src"
	...
	// JSON strict
	var js bytes.Buffer
	if err := runLint(&js, dir, true, false, true); err == nil {
		t.Fatalf("expected error under strict; out=%s", js.String())
	}
	// Human strict
	var hm bytes.Buffer
	if err := runLint(&hm, dir, false, false, true); err == nil {
		t.Fatalf("expected error under strict; out=%s", hm.String())
	}
