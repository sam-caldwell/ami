...
func TestLint_JSON_CompatCodes_FieldPresent(t *testing.T) {
    dir := filepath.Join("build", "test", "ami_lint", "compat_codes")
    ...
    setLintOptions(LintOptions{CompatCodes: true})
    ...
    for dec.More() {
        ...
        if data, ok := m["data"].(map[string]any); ok {
            if cc, ok2 := data["compatCode"].(string); ok2 && len(cc) > 5 {
                sawCompat = true
            }
        }
    }
    if !sawCompat { t.Fatalf("expected compatCode in JSON records; out=%s", buf.String()) }
}
