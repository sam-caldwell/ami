...
func testLint_RuleMapping_OffAndInfo(t *testing.T) {
	...
	if ws.Toolchain.Linter.Rules == nil {
		ws.Toolchain.Linter.Rules = map[string]string{}
	}
	...
	for dec.More() {
		...
		if m["code"] == "W_PKG_NAME_STYLE" && m["level"] == "info" {
			hasPkgInfo = true
		}
		if m["code"] == "W_IMPORT_SYNTAX" {
			hasImportSyntax = true
		}
	}
	if !hasPkgInfo {
		t.Fatalf("expected W_PKG_NAME_STYLE as info")
	}
	if hasImportSyntax {
		t.Fatalf("expected W_IMPORT_SYNTAX suppressed by rules")
	}
}
...
func testLint_PragmaDisable_UnknownIdent(t *testing.T) {
	...
	for dec.More() {
		...
		if m["code"] == "W_UNKNOWN_IDENT" {
			t.Fatalf("pragma failed to disable W_UNKNOWN_IDENT: %s", buf.String())
		}
	}
}
