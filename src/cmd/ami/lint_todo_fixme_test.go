...
func TestLint_TODO_FIXME_Warns(t *testing.T) {
    ...
    if err := os.WriteFile(filepath.Join(dir, "src", "main.ami"), src, 0o644); err != nil { t.Fatal(err) }
    var buf bytes.Buffer
    if err := runLint(&buf, dir, true, false, false); err != nil { t.Fatalf("unexpected error: %v\n%s", err, buf.String()) }
    out := buf.String()
    if !bytes.Contains([]byte(out), []byte("\"code\":\"W_TODO\"")) { t.Fatalf("expected W_TODO in output: %s", out) }
    if !bytes.Contains([]byte(out), []byte("\"code\":\"W_FIXME\"")) { t.Fatalf("expected W_FIXME in output: %s", out) }
}
...
func TestLint_TODO_PragmaDisable(t *testing.T) {
    ...
    if err := runLint(&buf, dir, true, false, false); err != nil { t.Fatalf("unexpected error: %v\n%s", err, buf.String()) }
    out := buf.String()
    if bytes.Contains([]byte(out), []byte("\"code\":\"W_TODO\"")) { t.Fatalf("did not expect W_TODO in output: %s", out) }
    if !bytes.Contains([]byte(out), []byte("\"code\":\"W_FIXME\"")) { t.Fatalf("expected W_FIXME in output: %s", out) }
}
