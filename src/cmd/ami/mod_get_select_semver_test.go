...
func testSelectHighestSemver_IncludingPrerelease(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("selectHighestSemver: %v", err)
	}
	if got != "v2.0.0-rc.1" {
		t.Fatalf("want v2.0.0-rc.1, got %s", got)
	}
}
...
func testSelectHighestSemver_ExcludePrerelease(t *testing.T) {
	...
	if got != "v1.10.0" {
		t.Fatalf("want v1.10.0, got %s", got)
	}
}

func testAtoi_WrapsStrconv(t *testing.T) {
	if _, err := atoi("123"); err != nil {
		t.Fatalf("atoi valid: %v", err)
	}
	if _, err := atoi("x"); err == nil {
		t.Fatalf("expected error for invalid atoi input")
	}
}
