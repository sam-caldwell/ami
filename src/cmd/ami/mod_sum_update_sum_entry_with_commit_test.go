...
func test_updateSumEntryWithCommit_arrayForm(t *testing.T) {
	...
	if mm["sha256"] != "deadbeef" {
		t.Fatalf("sha mismatch: %v", mm["sha256"])
	}
	if mm["commit"] != "cafebabe" {
		t.Fatalf("commit mismatch: %v", mm["commit"])
	}
	if mm["source"] != "git+ssh://example" {
		t.Fatalf("source mismatch: %v", mm["source"])
	}
}
...
func test_updateSumEntryWithCommit_objectForm(t *testing.T) {
	...
	if mm["version"] != "2.3.4" {
		t.Fatalf("version: %v", mm["version"])
	}
	if mm["sha256"] != "00ff" {
		t.Fatalf("sha: %v", mm["sha256"])
	}
	if mm["commit"] != "aa11" {
		t.Fatalf("commit: %v", mm["commit"])
	}
	if mm["source"] != "file+git:///abs/path" {
		t.Fatalf("source: %v", mm["source"])
	}
}
