...
func testModUpdate_LocalPackages_UpdateCacheAndSum_JSON(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(srcDir, "main.ami"), []byte("package app"), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	if err := os.WriteFile(filepath.Join(utilDir, "u.txt"), []byte("u"), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	...
	if len(res.Updated) != 2 {
		t.Fatalf("expected 2 updated, got %d", len(res.Updated))
	}
	// Validate cache contents
	if _, err := os.Stat(filepath.Join(cache, "app", "0.1.0", "main.ami")); err != nil {
		t.Fatalf("app not cached: %v", err)
	}
	if _, err := os.Stat(filepath.Join(cache, "util", "1.2.3", "u.txt")); err != nil {
		t.Fatalf("util not cached: %v", err)
	}
