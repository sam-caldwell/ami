...
func testModUpdate_ReportsSelectedVersions_ForRemoteReqs(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "src", "x.txt"), []byte("x"), 0o644); err != nil {
	...
	for _, s := range res.Selected {
		if s.Name == "lib" && s.Version == "v1.4.0" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected selection lib@v1.4.0; got: %+v", res.Selected)
	}
}
...
func testModUpdate_ReportsPrerelease_WhenConstraintHasPrerelease(t *testing.T) {
	...
	found := false
	for _, s := range res.Selected {
		if s.Name == "lib" && s.Version == "v1.4.0-rc.3" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected selection lib@v1.4.0-rc.3; got: %+v", res.Selected)
	}
}
