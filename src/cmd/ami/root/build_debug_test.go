...
`
    ...
    if err != nil { t.Fatalf("missing resolved.json: %v", err) }
    var sources sch.SourcesV1
    if err := json.Unmarshal(b, &sources); err != nil { t.Fatalf("unmarshal sources: %v", err) }
    if err := sources.Validate(); err != nil { t.Fatalf("sources validate: %v", err) }
    if len(sources.Units) == 0 { t.Fatalf("no source units recorded") }
    // ensure imports present
    gotImps := sources.Units[0].Imports
    expect := map[string]bool{"fmt":true, "foo/bar":true, "baz/qux":true}
    for k := range expect {
        found := false
        for _, v := range gotImps { if v == k { found = true; break } }
        if !found { t.Fatalf("expected import %s not found; got %v", k, gotImps) }
    }
    ...
    if asmPath == "" { t.Fatalf("did not find main.ami.s in asm index") }
    sha, size, err := fileSHA256Test(asmPath)
    if err != nil { t.Fatalf("compute sha: %v", err) }
    var rec sch.ASMFile
    for _, f := range idx.Files { if f.Path == asmPath { rec = f; break } }
    if rec.Path == "" { t.Fatalf("asm index record not found for %s", asmPath) }
    ...
    if err := man.Validate(); err != nil { t.Fatalf("manifest validate: %v", err) }
    var found bool
    ...
    if !found { t.Fatalf("manifest missing artifact for %s", asmPath) }
}
