...
`
	...
	for _, it := range edges.Items {
		if it.Label == "P.step2.in" && it.Kind == "edge.FIFO" {
			if it.Bounded {
				t.Fatalf("expected bounded=false; got true")
			}
			if it.Delivery != "bestEffort" {
				t.Fatalf("expected delivery=bestEffort; got %s", it.Delivery)
			}
			ok = true
			break
		}
	...
	if !ok {
		t.Fatalf("expected FIFO edge P.step2.in with bounded=false bestEffort in edges.json; got: %+v", edges.Items)
	}
}
