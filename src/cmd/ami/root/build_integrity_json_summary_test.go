...
`
	...
	if err := os.WriteFile(filepath.Join(entry, "f.txt"), []byte("x"), 0o644); err != nil {
	...
	if err := r.Storer.SetReference(plumbing.NewHashReference(plumbing.NewTagReferenceName(version), h)); err != nil {
		t.Fatalf("tag: %v", err)
	}
	...
	if err == nil {
		t.Fatalf("expected non-zero exit code; stdout=\n%s", string(out))
	}
	...
	for sc.Scan() {
		var d diag
		if json.Unmarshal([]byte(sc.Text()), &d) != nil {
			continue
		}
		if d.Schema == "diag.v1" && d.Level == "error" && d.Code == "E_INTEGRITY" {
			if !strings.Contains(d.Message, "integrity violation") {
	...
	if !seen {
		t.Fatalf("did not find summary E_INTEGRITY diag in output. stdout=\n%s", string(out))
	}
}
