...
func TestHelper_AmiBuildJSON(t *testing.T) {
    if os.Getenv("GO_WANT_HELPER_AMI_JSON") != "1" { return }
...
`
    ...
    for sc.Scan() {
        ...
        case "integrity: digest mismatch":
            seenMismatch = true
            if rec.Data == nil { t.Fatalf("missing data on digest mismatch record") }
            if rec.Data["pkg"].(string) != "example/repo" { t.Fatalf("pkg mismatch: %v", rec.Data["pkg"]) }
            if rec.Data["version"].(string) != version { t.Fatalf("version mismatch: %v", rec.Data["version"]) }
        }
    }
    if !seenMismatch {
