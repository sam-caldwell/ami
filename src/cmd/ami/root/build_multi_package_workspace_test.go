...
`
	...
	if _, err := os.Stat(filepath.Join("build", "debug", "ast", "main", "main.ami.ast.json")); err != nil {
		t.Fatalf("missing main ast: %v", err)
	}
	if _, err := os.Stat(filepath.Join("build", "debug", "ast", "util", "lib.ami.ast.json")); err != nil {
		t.Fatalf("missing util ast: %v", err)
	}

	// Expect obj outputs for both packages
	if _, err := os.Stat(filepath.Join("build", "obj", "main", "index.json")); err != nil {
		t.Fatalf("missing main obj index: %v", err)
	}
	if _, err := os.Stat(filepath.Join("build", "obj", "util", "index.json")); err != nil {
		t.Fatalf("missing util obj index: %v", err)
	}
}
