...
`
    ...
    if err == nil { t.Fatalf("expected non-zero exit 1; stdout=\n%s", string(out)) }
    if ee, ok := err.(*exec.ExitError); ok {
        if code := ee.ExitCode(); code != 1 { t.Fatalf("got exit %d want 1; stdout=\n%s", code, string(out)) }
    ...
    if count < 2 { t.Fatalf("expected multiple parser diagnostics; got %d; stdout=\n%s", count, string(out)) }
}
