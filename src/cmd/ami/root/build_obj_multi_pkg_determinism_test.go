...
`
	...
	for _, pkg := range []string{"main", "util"} {
		...
		if err != nil {
			t.Fatalf("missing obj index for %s: %v", pkg, err)
		}
		var idx1 sch.ObjIndexV1
		if json.Unmarshal(b, &idx1) != nil {
			t.Fatalf("unmarshal idx1 %s", pkg)
		}
		idx1.Timestamp = ""
		// read again after second run
		b2, err := os.ReadFile(idxPath)
		if err != nil {
			t.Fatalf("read idx2 %s: %v", pkg, err)
		}
		var idx2 sch.ObjIndexV1
		if json.Unmarshal(b2, &idx2) != nil {
			t.Fatalf("unmarshal idx2 %s", pkg)
		}
		idx2.Timestamp = ""
		if !reflect.DeepEqual(idx1, idx2) {
			t.Fatalf("obj index changed for %s", pkg)
		}
		...
		for _, f := range idx1.Files {
			...
			if !reflect.DeepEqual(c1, c2) {
				t.Fatalf("obj asm drift for %s", f.Path)
			}
		}
	}
}
