...
func TestHelper_AmiBuildJSON_Parse(t *testing.T) {
    if os.Getenv("GO_WANT_HELPER_AMI_BUILD_JSON_PARSE") != "1" { return }
...
`
    ...
    bad := "package 123\n"
    if err := os.WriteFile(filepath.Join(ws, "src", "main.ami"), []byte(bad), 0o644); err != nil { t.Fatalf("write src: %v", err) }
    ...
    for sc.Scan() {
        var r diagRec
        if json.Unmarshal([]byte(sc.Text()), &r) != nil { continue }
        if r.Schema == "diag.v1" && (r.Code == "E_PARSE" || r.Code == "E_BAD_PACKAGE" || r.Code == "E_BAD_PACKAGE_BLANK") { sawDiag = true; break }
    }
    if !sawDiag { t.Fatalf("expected parse-related diag in JSON; stdout=\n%s", string(out)) }
}
