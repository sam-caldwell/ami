...
`
	...
	if len(p.Pipelines) == 0 {
		t.Fatalf("expected at least one pipeline record")
	}
	// find pipeline P
	...
	if pipe == nil {
		t.Fatalf("expected pipeline named P in pipelines.json; got %+v", p.Pipelines)
	}
	if len(pipe.Steps) == 0 {
		t.Fatalf("expected at least one step in pipeline P")
	}
	...
	if !foundWorker {
		t.Fatalf("expected to find worker 'x' in pipeline P steps")
	}
	if worker.OutputKind == "" {
		t.Fatalf("expected worker 'x' to have outputKind populated")
	}
	if worker.Input == "" {
		t.Fatalf("expected worker 'x' to have input payload type populated")
	}
	if worker.Output == "" {
		t.Fatalf("expected worker 'x' to have output payload type populated")
	}
}
