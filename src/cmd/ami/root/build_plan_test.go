...
`
	...
	if err != nil {
		t.Fatalf("missing buildplan.json: %v", err)
	}
	...
	if len(plan.Targets) == 0 {
		t.Fatalf("expected at least one target in plan")
	}
...
`
	...
	for sc.Scan() {
		...
		if probe["schema"] == "buildplan.v1" {
			...
			if json.Unmarshal([]byte(line), &plan) == nil {
				if plan.Validate() == nil && len(plan.Targets) >= 1 {
					seen = true
					break
				}
			}
	...
	if !seen {
		t.Fatalf("did not observe buildplan.v1 JSON in output. got:\n%s", out)
	}
}
