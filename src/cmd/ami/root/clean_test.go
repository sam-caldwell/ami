...
func TestClean_Fresh_Human_EmitsActions(t *testing.T) {
	...
	if !strings.Contains(out, "clean.remove.skip") {
		t.Fatalf("missing remove.skip: %q", out)
	}
	if !strings.Contains(out, "clean.create") {
		t.Fatalf("missing create: %q", out)
	}
	// build dir exists
...
func TestClean_RemovesExistingFiles_Human(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join("build", "keep.txt"), []byte("x"), 0o644); err != nil {
	...
	if !strings.Contains(out, "clean.remove") {
		t.Fatalf("missing remove: %q", out)
	}
	if !strings.Contains(out, "clean.create") {
		t.Fatalf("missing create: %q", out)
	}
...
func TestClean_JSON_EmitsStructuredEvents(t *testing.T) {
	...
	for sc.Scan() {
		...
		if err := json.Unmarshal([]byte(sc.Text()), &r); err != nil {
			t.Fatalf("bad json: %v line=%q", err, sc.Text())
		}
		msgs = append(msgs, r.Message)
		if r.Data["path"] != "build" {
			t.Fatalf("expected path=build in data: %v", r.Data)
		}
	}
...
func TestClean_PermissionEdge_Human_EmitsError(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join("build", "f.txt"), []byte("x"), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	// Remove write permission from current working directory to induce RemoveAll failure
	if err := os.Chmod(".", 0o555); err != nil {
	...
	if _, err := os.Stat("build"); err != nil {
		t.Fatalf("expected build dir to remain on failure")
	}
}
