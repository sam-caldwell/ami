...
// Assert backpressure=drop is invalid (E_EDGE_BP_INVALID).
func TestLint_JSON_Backpressure_InvalidDrop(t *testing.T) {
...
`
    ...
    // Use backpressure=drop in an edge spec
    src := "package main\npipeline P { Ingress(cfg).Egress(in=edge.FIFO(minCapacity=0,maxCapacity=2,backpressure=drop,type=string)) }\n"
    ...
    for sc.Scan() {
        ...
        if obj["schema"] == "diag.v1" && obj["code"] == "E_EDGE_BP_INVALID" {
            seen = true
            break
        }
    }
    if !seen { t.Fatalf("expected E_EDGE_BP_INVALID in lint output; out=\n%s", out) }
}
