...
`
    ...
    for sc.Scan() {
        ...
        if json.Unmarshal([]byte(line), &obj) != nil { continue }
        ...
        case "W_PKG_LOWERCASE":
            if lvl, _ := obj["level"].(string); lvl == "error" { seenLowerAsError = true }
        }
    }
    if seenUnused { t.Fatalf("unexpected W_UNUSED_IMPORT in output (should be suppressed).\n%s", out) }
    if seenNoNewline { t.Fatalf("unexpected W_FILE_NO_NEWLINE in output (should be off).\n%s", out) }
    if !seenLowerAsError { t.Fatalf("expected W_PKG_LOWERCASE with level error.\n%s", out) }
}
