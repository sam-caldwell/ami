...
`
	...
	if err := os.MkdirAll(filepath.Join("lib", "util"), 0o755); err != nil {
		t.Fatalf("mkdir lib/util: %v", err)
	}
	...
	for sc.Scan() {
		...
		if obj["schema"] == "diag.v1" && obj["code"] == "E_IMPORT_PRERELEASE_FORBIDDEN" {
			seen = true
			break
		}
	}
	if !seen {
		t.Fatalf("expected E_IMPORT_PRERELEASE_FORBIDDEN in output; got:\n%s", out)
	}
}
...
`
	...
	if err := os.MkdirAll(filepath.Join("app", "other"), 0o755); err != nil {
		t.Fatalf("mkdir app/other: %v", err)
	}
	if err := os.MkdirAll(filepath.Join("lib", "util"), 0o755); err != nil {
		t.Fatalf("mkdir lib/util: %v", err)
	}
	...
	for sc.Scan() {
		...
		if obj["schema"] == "diag.v1" && obj["code"] == "E_IMPORT_CONSISTENCY" {
			seen = true
			break
		}
	}
	if !seen {
		t.Fatalf("expected E_IMPORT_CONSISTENCY in output; got:\n%s", out)
	}
}
