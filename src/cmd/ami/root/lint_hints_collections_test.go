...
`
	...
	for sc.Scan() {
		...
		if obj["schema"] == "diag.v1" && obj["code"] == "W_MAP_KEY_TYPE_HINT" {
			seen = true
			break
		}
	}
	if !seen {
		t.Fatalf("expected W_MAP_KEY_TYPE_HINT in output; got:\n%s", out)
	}
}
...
`
	...
	for sc.Scan() {
		...
		if obj["schema"] == "diag.v1" && obj["code"] == "W_SET_ELEM_TYPE_HINT" {
			seen = true
			break
		}
	}
	if !seen {
		t.Fatalf("expected W_SET_ELEM_TYPE_HINT in output; got:\n%s", out)
	}
}
...
`
	...
	for sc.Scan() {
		...
		if obj["schema"] == "diag.v1" && obj["code"] == "W_SLICE_ARITY_HINT" {
			seen = true
			break
		}
	}
	if !seen {
		t.Fatalf("expected W_SLICE_ARITY_HINT in output; got:\n%s", out)
	}
}
