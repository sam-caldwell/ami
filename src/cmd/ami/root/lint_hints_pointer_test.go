...
func TestLint_Hints_PointerParam_NilGuardInfo(t *testing.T) {
    tmp := t.TempDir()
    t.Setenv("HOME", tmp)
...
`
    ...
    for sc.Scan() {
        ...
        if obj["schema"] == "diag.v1" && obj["code"] == "W_PTR_TYPE_HINT" {
            if lvl, _ := obj["level"].(string); lvl == "info" { seen = true; break }
        }
    }
    if !seen { t.Fatalf("expected W_PTR_TYPE_HINT info in output; got:\n%s", out) }
}
