...
`
	...
	if err != nil {
		t.Fatalf("unexpected non-zero exit: %v; out=\n%s", err, string(out))
	}
	...
	for sc.Scan() {
		...
		if code == "W_FILE_CRLF" {
			seenCRLF = true
		} else if code != "LINT_SUMMARY" {
			seenOther = true
		}
	}
	if !seenCRLF || seenOther {
		t.Fatalf("rules filter failed; out=\n%s", string(out))
	}
}
...
`
	...
	for sc.Scan() {
		...
		if obj["schema"] != "diag.v1" || obj["code"] == "LINT_SUMMARY" {
			continue
		}
		code, _ := obj["code"].(string)
...
`
	...
	if warns > 2 {
		t.Fatalf("expected at most 2 warnings, got %d\n%s", warns, string(out))
	}
}
