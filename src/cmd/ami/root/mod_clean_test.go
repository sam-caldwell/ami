...
func TestModClean_Fresh_Human(t *testing.T) {
	...
	if fi, err := os.Stat(dir); err != nil || !fi.IsDir() {
		t.Fatalf("cache dir not created: %s", dir)
	}
}
...
func TestModClean_Existing_Human(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "x.txt")); err == nil {
		t.Fatalf("expected file removed by mod clean")
	}
}
...
func TestModClean_JSON(t *testing.T) {
	...
	for sc.Scan() {
		...
		if err := json.Unmarshal([]byte(sc.Text()), &r); err != nil {
			t.Fatalf("invalid json: %v line=%q", err, sc.Text())
		}
		if r.Data["path"] == "" {
			t.Fatalf("expected path in data: %v", r.Data)
		}
...
func TestModClean_PermissionEdge_Error(t *testing.T) {
	...
	if _, e := os.Stat(dir); e != nil {
		t.Fatalf("expected cache dir to remain on failure")
	}
}
