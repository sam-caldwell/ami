...
func TestModList_JSON_IncludesManifestInfoWhenAvailable(t *testing.T) {
  ...
  "createdAt": "2025-01-01T00:00:00.000Z"
    ...
    if err := os.WriteFile(filepath.Join(ws, "ami.manifest"), []byte(man), 0o644); err != nil { t.Fatalf("write manifest: %v", err) }
    ...
    for sc.Scan() {
        ...
        if rec.Message != "cache.entry" { continue }
        if rec.Data["entry"] != base+"@"+ver { continue }
        if rec.Data["manifest"].(bool) != true { t.Fatalf("expected manifest=true") }
        if rec.Data["manifestName"].(string) != full { t.Fatalf("manifestName mismatch: %v", rec.Data["manifestName"]) }
        if rec.Data["manifestDigest"].(string) != digest { t.Fatalf("manifestDigest mismatch: %v", rec.Data["manifestDigest"]) }
        if rec.Data["manifestSource"].(string) != entry { t.Fatalf("manifestSource mismatch: %v", rec.Data["manifestSource"]) }
        seen = true
    }
    if !seen { t.Fatalf("did not see expected cache.entry record") }
}
