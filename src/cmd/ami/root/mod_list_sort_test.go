...
func TestModList_JSON_SortedOrder(t *testing.T) {
    ...
    if err := os.MkdirAll(filepath.Join(cache, "b@v1.0.0"), 0o755); err != nil { t.Fatalf("mkdir: %v", err) }
    if err := os.MkdirAll(filepath.Join(cache, "a@v1.0.0"), 0o755); err != nil { t.Fatalf("mkdir: %v", err) }
    if err := os.MkdirAll(filepath.Join(cache, "c@v1.0.0"), 0o755); err != nil { t.Fatalf("mkdir: %v", err) }
