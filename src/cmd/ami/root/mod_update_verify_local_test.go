...
func makeLocalRepo(t *testing.T, path string) (string, string) {
    ...
    // commit 1
    if err := os.WriteFile(filepath.Join(path, "file.txt"), []byte("one"), 0o644); err != nil { t.Fatalf("write: %v", err) }
    w, _ := r.Worktree()
    _, _ = w.Add("file.txt")
    h1, err := w.Commit("c1", &git.CommitOptions{Author: &object.Signature{Name: "t", Email: "t@x", When: time.Now()}})
    if err != nil { t.Fatalf("commit1: %v", err) }
    ...
    // commit 2
    if err := os.WriteFile(filepath.Join(path, "file.txt"), []byte("two"), 0o644); err != nil { t.Fatalf("write2: %v", err) }
    _, _ = w.Add("file.txt")
    h2, err := w.Commit("c2", &git.CommitOptions{Author: &object.Signature{Name: "t", Email: "t@x", When: time.Now().Add(time.Second)}})
    if err != nil { t.Fatalf("commit2: %v", err) }
...
`
    ...
    if _, err := os.Stat(sumPath); err != nil { t.Fatalf("ami.sum missing: %v", err) }
    ...
    if err != nil { t.Fatalf("load sum: %v", err) }
    var found bool
    for pkg, vers := range sum.Packages {
        ...
        if filepath.Base(pkg) == "example" {
            if d, ok := vers[latest]; ok {
                ...
                if d != d2 { t.Fatalf("digest mismatch: sum=%s computed=%s", d, d2) }
            }
        }
    }
    if !found { t.Fatalf("did not find example@%s in ami.sum", latest) }
