...
func runAmiTests(pass, fail, skip *int, get func(pkg string) *ctot) {
	...
	for _, file := range files {
		...
		pkgName := f.Package
		if pkgName == "" {
			pkgName = uPackageFromPath(file, roots)
		}
		if pkgName == "" {
			pkgName = "main"
		}
		...
		for _, c := range cases {
			...
			if c.skipReason != "" {
				if flagJSON {
					...
					logger.Warn(fmt.Sprintf("test SKIP %s %s", c.pkg, c.name), nil)
				}
				*skip++
				get(c.pkg).skip++
				continue
			...
			if ok {
				if flagJSON {
					...
					logger.Info(fmt.Sprintf("test PASS %s %s", c.pkg, c.name), nil)
				}
				*pass++
				get(c.pkg).pass++
			} else {
				if flagJSON {
					...
					for _, d := range failDiags {
						dlist = append(dlist, d.ToSchema())
					}
					...
					logger.Error(fmt.Sprintf("test FAIL %s %s", c.pkg, c.name), nil)
				}
				*fail++
				get(c.pkg).fail++
			}
		...
		if len(rcases) > 0 {
			...
			for _, rc := range rcases {
				...
				for _, f := range rc.fixtures {
					fx = append(fx, tester.Fixture{Path: f.path, Mode: f.mode})
				}
...
func deriveAmiCases(file, pkg string, f *astpkg.File) []amiCase {
	...
	for _, d := range f.Directives {
		...
		case "test:expect_no_errors":
			if cur.name == "" {
				cur = amiCase{name: filepath.Base(file), file: file, pkg: pkg}
			}
