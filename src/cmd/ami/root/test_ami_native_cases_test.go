...
`
	...
	if err := os.WriteFile(filepath.Join(ws, "src", "a_test.ami"), []byte(a), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(ws, "src", "b_test.ami"), []byte(b), 0o644); err != nil {
		t.Fatalf("write b_test: %v", err)
	}
	// Case 3: skip
	c := "#pragma test:case skip_case\n#pragma test:skip not_implemented\npackage main\n"
	if err := os.WriteFile(filepath.Join(ws, "src", "c_test.ami"), []byte(c), 0o644); err != nil {
		t.Fatalf("write c_test: %v", err)
	}
	...
	if err != nil {
		if ee, ok := err.(*exec.ExitError); ok {
			if code := ee.ExitCode(); code != 1 {
				t.Fatalf("unexpected exit code: %d; stdout=\n%s", code, string(out))
			}
		} else {
