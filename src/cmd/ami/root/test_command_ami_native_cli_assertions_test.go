...
import "bad path"
`
	if err := os.WriteFile(filepath.Join(ws, "src", "count_test.ami"), []byte(countCase), 0o644); err != nil {
...
import "bad path"
`
	if err := os.WriteFile(filepath.Join(ws, "src", "substr_test.ami"), []byte(substrCase), 0o644); err != nil {
		t.Fatalf("write substr: %v", err)
	}
	...
	for sc.Scan() {
		...
		if obj["schema"] == "test.v1" && obj["type"] == "test_end" && obj["status"] == "pass" {
			if name, _ := obj["name"].(string); want[name] == false {
				want[name] = true
			}
		}
	}
	for k, v := range want {
		if !v {
			t.Fatalf("expected pass for %s; output=\n%s", k, string(out))
		}
	}
}
