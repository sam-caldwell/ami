...
Pipeline(P) { }
`
    if err := os.WriteFile(filepath.Join(ws, "src", "rt2_test.ami"), []byte(a), 0o644); err != nil { t.Fatalf("write rt2: %v", err) }
    ...
    for sc.Scan() {
        ...
        if obj["schema"] == "test.v1" && obj["type"] == "test_end" && obj["status"] == "pass" {
            ...
            if _, ok := want[name]; ok { want[name] = true }
        }
    }
    for k, v := range want { if !v { t.Fatalf("expected pass for %s; stdout=\n%s", k, string(out)) } }
}
