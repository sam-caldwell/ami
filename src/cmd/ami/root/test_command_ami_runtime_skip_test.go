...
Pipeline(P) { }
`
	if err := os.WriteFile(filepath.Join(ws, "src", "rt1_test.ami"), []byte(a), 0o644); err != nil {
		t.Fatalf("write rt1: %v", err)
	}
	...
	for sc.Scan() {
		...
		if obj["schema"] == "test.v1" && obj["type"] == "test_end" && obj["status"] == "pass" {
			if name, _ := obj["name"].(string); name == "RunEcho" {
				sawPass = true
			}
		}
	}
	if !sawPass {
		t.Fatalf("expected pass for runtime case; stdout=\n%s", string(out))
	}
}
