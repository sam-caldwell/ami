...
`
    if err := os.WriteFile(filepath.Join(ws, "p1", "a_test.go"), []byte(src), 0o644); err != nil { t.Fatalf("write p1 test: %v", err) }
    if err := os.WriteFile(filepath.Join(ws, "p2", "a_test.go"), []byte(strings.Replace(src, "package p", "package q", 1)), 0o644); err != nil { t.Fatalf("write p2 test: %v", err) }
    ...
    type runStart struct{ Schema, Type string; PkgParallel int `json:"pkg_parallel"` }
    sc := bufio.NewScanner(strings.NewReader(string(out)))
    for sc.Scan() {
        var rs runStart
        if json.Unmarshal([]byte(sc.Text()), &rs) != nil { continue }
        if rs.Schema == "test.v1" && rs.Type == "run_start" {
            if rs.PkgParallel != 2 { t.Fatalf("expected pkg_parallel=2; got %d", rs.PkgParallel) }
            return
        }
    }
