...
`
	if err := os.WriteFile(filepath.Join(ws, "p1", "a_test.go"), []byte(src1), 0o644); err != nil {
		t.Fatalf("write p1: %v", err)
	}
	if err := os.WriteFile(filepath.Join(ws, "p2", "a_test.go"), []byte(src2), 0o644); err != nil {
	...
	if err != nil {
		t.Fatalf("unexpected exit: %v; stdout=\n%s", err, string(out))
	}

	type pkgSummary struct {
		Package                 string
		Pass, Fail, Skip, Cases int
	}
	type runEnd struct {
	...
	for sc.Scan() {
		...
		if json.Unmarshal([]byte(sc.Text()), &probe) != nil {
			continue
		}
		if probe["schema"] == "test.v1" && probe["type"] == "run_end" {
			if json.Unmarshal([]byte(sc.Text()), &seen) == nil {
				break
			}
		}
	}
	if len(seen.Packages) < 2 {
		t.Fatalf("expected >=2 package summaries; got %d", len(seen.Packages))
	}
	...
	for _, p := range seen.Packages {
		if p.Package == want1 && p.Cases == 1 && p.Pass == 1 && p.Fail == 0 && p.Skip == 0 {
			ok1 = true
		}
		if p.Package == want2 && p.Cases == 1 && p.Pass == 1 && p.Fail == 0 && p.Skip == 0 {
			ok2 = true
		}
	}
	if !ok1 || !ok2 {
