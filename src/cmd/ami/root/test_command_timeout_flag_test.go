...
`
    if err := os.WriteFile(filepath.Join(ws, "a_test.go"), []byte(slow), 0o644); err != nil { t.Fatalf("write test: %v", err) }
    ...
    if err == nil { t.Fatalf("expected non-zero exit (2) on timeout; stdout=\n%s", string(out)) }
    if ee, ok := err.(*exec.ExitError); ok {
        if code := ee.ExitCode(); code != 2 { t.Fatalf("unexpected exit code: got %d want 2", code) }
    } else { t.Fatalf("unexpected error type: %T", err) }
    ...
    if !sawStart || !sawEnd { t.Fatalf("expected test.v1 run_start/run_end events; got:\n%s", string(out)) }
}
