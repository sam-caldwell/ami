...
func newRootCmd() *cobra.Command {
    ...
    cmd.PersistentPreRunE = func(cmd *cobra.Command, _ []string) error {
        ...
        if json && color { return exit.New(exit.User, "--json and --color cannot be used together") }
        opts := logging.Options{ JSON: json, Verbose: verbose, Color: color && !json, Package: "cmd/ami", Out: io.Discard, DebugDir: "build/debug", RedactKeys: redact, RedactPrefixes: redactPrefixes, FilterAllowKeys: allowFields, FilterDenyKeys: denyFields }
        lg, err := logging.New(opts)
        if err == nil { setRootLogger(lg) }
        if err := ensurePackageCache(); err != nil { return err }
        return nil
    }
