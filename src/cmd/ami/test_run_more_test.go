...
`
	if err := os.WriteFile(filepath.Join(dir, "tmp_test.go"), []byte(testSrc), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	var buf bytes.Buffer
	if err := runTest(&buf, dir, true, false, 0); err != nil {
...
`
	if err := os.WriteFile(filepath.Join(dir, "a", "a_test.go"), []byte(aSrc), 0o644); err != nil {
		t.Fatalf("write a: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "b", "b_test.go"), []byte(bSrc), 0o644); err != nil {
		t.Fatalf("write b: %v", err)
	}
	var buf bytes.Buffer
	if err := runTest(&buf, dir, true, false, 2); err != nil {
		t.Fatalf("runTest: %v", err)
	}
...
func testRunTest_AMI_Directives_ParseFail_PositionAndCount(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "main.ami"), []byte(src), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	var buf bytes.Buffer
	if err := runTest(&buf, dir, true, false, 0); err != nil {
...
`
	...
	if !strings.Contains(out.String(), `"ok":`) {
		t.Fatalf("missing JSON summary: %s", out.String())
	}
	...
	if _, err := os.Stat(logPath); err != nil {
		t.Fatalf("test.log missing: %v", err)
	}
	if _, err := os.Stat(manPath); err != nil {
		t.Fatalf("test.manifest missing: %v", err)
	}
...
func testRunTest_JSON_AmiEventsContainFields(t *testing.T) {
	...
	if !found {
		t.Fatalf("ami.test.v1 event missing or malformed: %s", buf.String())
	}
}
...
`
	...
	if err == nil {
		t.Fatalf("expected error from failing tests")
	}
	// Parse summary
	lines := bytes.Split(bytes.TrimSpace(buf.Bytes()), []byte("\n"))
	var last map[string]any
	...
	if ok, _ := last["ok"].(bool); ok {
		t.Fatalf("expected ok=false, got ok=true: %v", last)
	}
	if int(last["failures"].(float64)) < 1 {
		t.Fatalf("expected failures>=1: %v", last)
	}
}
...
func testRunTest_Verbose_AmiManifestOrder(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "b.ami"), []byte(b), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "a.ami"), []byte(a), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	if err := runTest(io.Discard, dir, false, true, 0); err != nil {
		t.Fatalf("runTest: %v", err)
	}
	...
	for _, ln := range lines {
		if strings.HasPrefix(ln, "ami:") {
			amis = append(amis, ln)
		}
	}
	if len(amis) != 2 {
		t.Fatalf("expected two ami entries, got: %v", amis)
	}
