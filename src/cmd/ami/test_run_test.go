...
`
	...
	if err := runTest(os.Stdout, dir, false, true, 0); err != nil {
		t.Fatalf("runTest: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, "build", "test", "test.log")); err != nil {
		t.Fatalf("test.log missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, "build", "test", "test.manifest")); err != nil {
...
`
	...
	if ok, _ := last["ok"].(bool); !ok {
		t.Fatalf("ok=false in summary: %v", last)
	}
...
`
	...
	if !strings.Contains(s, `"Action":"run"`) {
		t.Fatalf("expected streamed events in JSON output: %s", s)
	}
	lines := bytes.Split(bytes.TrimSpace(buf.Bytes()), []byte("\n"))
	var last map[string]any
	if err := json.Unmarshal(lines[len(lines)-1], &last); err != nil {
	...
	if ok, _ := last["ok"].(bool); !ok {
		t.Fatalf("expected ok=true summary; last=%v", last)
	}
	if int(last["packages"].(float64)) != 1 || int(last["tests"].(float64)) != 1 || int(last["failures"].(float64)) != 0 {
		t.Fatalf("unexpected counts: %v", last)
	}
...
`
	...
	if err := runTest(os.Stdout, dir, false, false, 0); err == nil {
		t.Fatalf("expected error from failing test run")
	}
}
...
func testRunTest_AMIDirectives_ParseOK_And_Manifest(t *testing.T) {
	...
	var buf bytes.Buffer
	if err := runTest(&buf, dir, true, true, 0); err != nil {
		t.Fatalf("runTest: %v", err)
	}
	...
	if !strings.Contains(buf.String(), `"schema":"ami.test.v1"`) {
		t.Fatalf("missing ami JSON event: %s", buf.String())
	}
}
