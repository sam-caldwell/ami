...
func TestEventMetaV1_ValidateAndMarshal(t *testing.T) {
	...
	if got.Schema != "eventmeta.v1" || !got.ImmutablePayload || len(got.Fields) < 3 {
		t.Fatalf("round-trip mismatch: %+v", got)
	}
	if got.Trace == nil || got.Trace.Traceparent.Type != "string" {
		t.Fatalf("missing trace context: %+v", got.Trace)
	}
}
